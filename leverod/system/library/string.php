...
/**
 * Random string generator (used for generating random css id/class names)
 *
 * @param int	$length  	URI string
 * @param bool	$uppercase	Allow uppercase 
 * @param bool	$numbers	Allow numbers
 * 
 * @return string
 */ 
...
if (!function_exists('random_string')) {
	...
	function random_string($length = 8, $uppercase = false, $numbers = false) {
	
		$lc = 'abcdefghijklmnopqrstuvwxyz';
		$uc = $uppercase? 'ABCDEFGHIJKLMNOPQRSTUVWXYZ' : '';
		$n	= $numbers? '0123456789': '';

		return substr(str_shuffle($lc . $uc . $n), 0, $length);
	}
}	
...
if (!function_exists('format_uri')) {	 
	function format_uri( $uri, $separator = '-' ) {
		$accents_regex = '~&([a-z]{1,2})(?:acute|cedil|circ|grave|lig|orn|ring|slash|th|tilde|uml);~i';
		$special_cases = array( '&' => 'and', "'" => '');
		$uri = utf8_strtolower( trim( $uri ), 'UTF-8' );
		$uri = str_replace( array_keys($special_cases), array_values( $special_cases), $uri );
		$uri = preg_replace( $accents_regex, '$1', htmlentities( $uri, ENT_QUOTES, 'UTF-8' ) );
		$uri = preg_replace("/[^a-z0-9]/u", "$separator", $uri);
		$uri = preg_replace("/[$separator]+/u", "$separator", $uri);
		return $uri;
	}	
}
...
if (!function_exists('sanitize')) {	 
	function sanitize($string) {
		...
		$string = html_entity_decode($string); // String contains $this->request->post['name'] from the product page which is entity encoded
		$string = str_replace(array('&',   '=',   '^',   '%',   '[', ']',   '#',   '*',   '§',   '<',   '>',   '@',   '°',   '+',  '|',   '$',   '€',   '(',   ')',   '\"',   '!',   '?',   '\'',   ';', ':', ',', '.',  '_',   '\\', '/', '"', ' '), '-', utf8_strtolower($string, 'UTF-8'));		
		...
		$string = trim(preg_replace('/(-){1,}/', '-',$string),'-'); // trim removes the - from the begging and end of the string
...
/**
 * Kb - Mb - Gb String notation to bytes 
 *
 * @param string	$val 		String to convert, the size can be written with one or two letters (M or MB)
 * 
 * @return int
 */ 
 ...
 if (!function_exists('stringToBytes')) {	
	function stringToBytes($val) {
		...
		$val	= trim($val);
		$offset = !is_numeric($val[strlen($val)-2])? 2 : 1; // check if the size is expressed with one or two letters (M or MB)
		$last	= strtolower($val[strlen($val) - $offset]);
		$val	= substr($val, 0, -$offset); // necessary since PHP 7.1; otherwise optional
...
// Utf-8 support (functions taken from /system/helper/utf8.php)

if (!function_exists('utf8_strlen')) { // Check whether Opencart utf8 strings have already been loaded
