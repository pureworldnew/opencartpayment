// ***************************************************
//       Jquery Minicolors + Graphics functions
//       
...
function is_valid_hex_color(hex) { // It tests 3 or 6 char hex colors
...
function style_color_picker_input(input, hex){
	...
	$(input).parent().parent().find('.stars *').css('color', '#' + hex);
...
function show_warning(elem){
	elem.next('i').remove();
	elem.after('<i class="fa fa-exclamation-triangle"></i>');
}
