...
class SmushIt
{
	...
	/**
	 * Array used in the clean method - iSenseLabs modification
	 * @access private
	 * @var array
	 */
	private $clean = array();
	...
	private function clean($sources)
	{
		if (is_array($sources)) {
			$this->clean = array();
			array_walk_recursive($sources, array($this, 'clean_walkrecursive_anonymous'));
			$sources = array_filter(array_map(array($this, 'clean_map_anonymous'), array_unique($this->clean)));
	...
	/**
	 * Extracted from the default clean method for compatibility with older php versions
	 */
	private function clean_walkrecursive_anonymous($line) {
		$this->clean[] = $line;
	}
	 ...
	 * Extracted from the default clean method for compatibility with older php versions
	 */
	private function clean_map_anonymous($line) {
	...
	private function smush()
	{
		...
		if (!empty($result['error'])) {
			throw new Exception($result['error'] . ' | Image: ' . $result['src']);
			return;
		}
