...
class ModelPosPos extends Model {
    ...
    public function getOrderPaidAmount($order_id) {
        $paid = 0.00;
        $sql = "SELECT * FROM " . DB_PREFIX . "order_payment WHERE order_id = '" . $order_id . "'";
        $query = $this->db->query($sql);
        if ($query->rows) {
            foreach ($query->rows as $row) {
                if ($row['payment_type'] != 'pos_change') {
                    $paid += $row['tendered_amount'];
                }
            }
        }
        return $paid;
    ...
    public function getOrderChangeAmount($order_id) {
        $change = 0.00;
        $sql = "SELECT * FROM " . DB_PREFIX . "order_payment WHERE order_id = '" . $order_id . "'";
        $query = $this->db->query($sql);
        if ($query->rows) {
    ...
    /* public function getOrderChangeAmount($order_id)
      {
      $change = 0.00;
      $order_total = $this->getOrderTotalAmount($order_id);
      $order_paid =  $this->getOrderPaidAmount($order_id);

      if( $order_paid > $order_total )
      {
      $change = $order_paid - $order_total;
      }
      return $change;
      } */
    ...
    public function getOrderTotalAmount($order_id) {
        ...
        $sql = "SELECT * FROM " . DB_PREFIX . "order_total WHERE order_id = '" . $order_id . "' AND code = 'total'";
        $query = $this->db->query($sql);
        return $query->row ? $query->row['value'] : 0.00;
    }

    public function getOrderDueAmount($order_id) {

        $sql = "SELECT * FROM " . DB_PREFIX . "order_total WHERE order_id = '" . $order_id . "' AND code = 'total'";
        $query = $this->db->query($sql);
        $total = $query->row ? $query->row['value'] : 0.00;
        $paid = 0.00;
        $sql = "SELECT * FROM " . DB_PREFIX . "order_payment WHERE order_id = '" . $order_id . "'";
        $query = $this->db->query($sql);
        if ($query->rows) {
    ...
    public function addOrderPayment($data) {
        if (isset($data['order_id'])) {
            ...
            $this->db->query("UPDATE `" . DB_PREFIX . "order` SET payment_code='" . $this->db->escape($data['payment_code']) . "',payment_method='" . $this->db->escape($data['payment_type']) . "' WHERE order_id='" . (int) $data['order_id'] . "' ");
    ...
    public function checkOrderHasTax($order_id) {
        $sql = "SELECT * FROM " . DB_PREFIX . "order_total WHERE order_id = '" . $order_id . "' AND code = 'tax'";
        $query = $this->db->query($sql);
        return $query->row ? true : false;
    }

    public function getCurrentOrderTotal($order_id, $code) {
        $sql = "SELECT value FROM " . DB_PREFIX . "order_total WHERE order_id = '" . $order_id . "' AND code = '" . $code . "'";
        $query = $this->db->query($sql);
        return $query->row ? $query->row['value'] : false;
    }

    public function getOrderDiscountAmount($order_id) {
        $sql = "SELECT value FROM " . DB_PREFIX . "order_total WHERE order_id = '" . $order_id . "' AND code LIKE '%pos_discount_%'";
        $query = $this->db->query($sql);
        return $query->row ? $query->row['value'] * -1 : 0;
    }

    public function applyDiscountLevel($order_product_id, $price) {
        $sql = "SELECT unit_conversion_values FROM " . DB_PREFIX . "order_product WHERE order_product_id = '" . (int) $order_product_id . "'";
        $query = $this->db->query($sql);
        $unit_conversion_values = $query->row ? $query->row['unit_conversion_values'] : 0;
        if ($unit_conversion_values > 0) {
            $sql = "SELECT convert_price FROM " . DB_PREFIX . "unit_conversion_product WHERE unit_conversion_product_id = '" . (int) $unit_conversion_values . "'";
            $query = $this->db->query($sql);
            $convert_price = $query->row ? $query->row['convert_price'] : 1;
        } else {
            $convert_price = 1;
        }
        $price = $price * $convert_price;
        $this->db->query("UPDATE " . DB_PREFIX . "order_product SET price = '" . (float) $price . "', total = quantity * '" . (float) $price . "'
		WHERE order_product_id = '" . (int) $order_product_id . "'");
    }

    public function getProductNormalPrice($product_id) {
    ...
    public function getOrderPaymentStatus($order_id) {
        ...
        if ($query->rows) {
            ...
            if ($total > 0 && $total <= $paid) {
                return "Paid: " . $payment_type . " ($" . number_format((float) $paid, 2, '.', '') . ")";
            } else {
                return "Partial Paid: " . $payment_type . " ($" . number_format((float) $paid, 2, '.', '') . ")";
            }
        } else {
            return "Not Paid";
        }
    ...
    public function getAddressByCustomerId($customer_id) {
        $sql = "SELECT * FROM " . DB_PREFIX . "address WHERE customer_id = '" . $customer_id . "' LIMIT 1";
        $query = $this->db->query($sql);
        $address = $query->row;
        if ($address) {
    ...
    public function saveOrderStatus($order_id, $order_status_id) {
        $sql = "UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . (int) $order_status_id . "',is_pos='1'";
        // add for Sale Person Affiliate begin
    ...
    public function updateCustomerGroup($customer_group_id, $customer_id) {
        if ($customer_id > 0) {
    ...
    public function getOrders($data = array()) {
        $sql = "SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, o.sales_person ,o.user_id, o.order_status_id, o.email, t.name, ";
        // add for Quotation begin
        ...
        $sql .= "(SELECT CONCAT(u.firstname, ' ', u.lastname) as name  FROM " . DB_PREFIX . "user u WHERE u.user_id = o.sales_person) AS salesperson, ";
        ...
        if (!empty($data['filter_order_saleper_id'])) {
            $sql .= " AND o.sales_person = '" . (int) $data['filter_order_saleper_id'] . "'";
        }
    ...
    public function getTotalOrders($data = array()) {
        ...
        if (!empty($data['filter_order_saleper_id'])) {
            $sql .= " AND sales_person = '" . (int) $data['filter_order_saleper_id'] . "'";
        }
        // add for user can only manage his/her own orders end
        // add for Quotation begin
        if (!empty($data['filter_quote'])) {
    ...
    public function orderHasShipping($order_id) {
    ...
    public function UpdateOrderShipping($order_id, $data) {

        if ($data['shipping'] == 'free.free' && !empty($data['shipping_title']) && !empty($data['shipping_cost'])) {
            $title = $data['shipping_title'];
            $value = $data['shipping_cost'];
            if ($this->orderHasShipping($order_id)) {
                $this->db->query("UPDATE " . DB_PREFIX . "order_total SET title='" . $title . "', value=" . $value . " WHERE order_id='" . $order_id . "' AND code='shipping'");
            } else {
                $this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET title='" . $title . "', value=" . $value . ",order_id='" . $order_id . "',sort_order=3,code='shipping'");
            }
        } else {

            if ($this->orderHasShipping($order_id) && $data['shipping'] != 'free.free') {
                $this->db->query("UPDATE " . DB_PREFIX . "order_total SET title='Shipping', value=0 WHERE order_id='" . $order_id . "' AND code='shipping'");
            } else {
                if (!$this->orderHasShipping($order_id)) {
                    $this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET title='Shipping',value=0,order_id='" . $order_id . "',sort_order=3,code='shipping'");
                }
            }
        }
        //exit;
    ...
    public function getProductsForBrowse($data) {
        $sql = "SELECT p.product_id, p.price, p.model, p.shipping, p.tax_class_id, p.quantity, p.subtract, p.image, p.weight_price, p.weight_name, p.points, p.upc, p.sku, p.ean, p.isbn, p.mpn, p.quick_sale, p.labour_cost, p.unique_option_price, pd.name, pd.description, pm.name AS m_name FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN `" . DB_PREFIX . "manufacturer` pm ON p.manufacturer_id = pm.manufacturer_id WHERE p.pos_status='1' AND pd.language_id = '" . (int) $this->config->get('config_language_id') . "'";

        if (!empty($data['filter_name']) && !empty($data['filter_scopes'])) {
            //$sql .= " AND (pd.name LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
            /* $words = explode(' ', trim(preg_replace('/\s+/', ' ', $data['filter_name'])));
              foreach ($words as $word) {
              $sql .=" OR pd.name LIKE '%" . $this->db->escape($word) . "%'";
              } */
            ...
            if (isset($words[1]) && !empty($words[1])) {
                $strictEqual = true;
                //$words[1] = preg_replace("/[^A-Za-z0-9_\s-]/", "", $words[1]);
                $sql .= " pd.name = '" . $this->db->escape($words[1]) . "'";
            } else {
                $strictEqual = false;
            }
            ...
            if (!$strictEqual) {
                ...
                if (isset($words[0]) && !empty($words[0])) {
                    if (isset($words[1]) && !empty($words[1])) {
                        $strictAnd = true;
                        $words[0] = preg_replace("/[^A-Za-z0-9_\s-]/", "", $words[0]);
                        $words[1] = preg_replace("/[^A-Za-z0-9_\s-]/", "", $words[1]);

                        $implode = array();
                        $words_inner = explode(' ', trim(preg_replace('/\s+/', ' ', $words[0])));
                        foreach ($words_inner as $word_inner) {
                            $implode[] = "pd.name LIKE '%" . $this->db->escape($word_inner) . "%'";
                        }
                        if ($implode) {
                        ...
                        foreach ($data['filter_scopes'] as $filter_scope) {
                            if ($filter_scope == 'model') {
                                $sql_sub .= "LOWER(p.model) LIKE LOWER('%" . $this->db->escape($words[1]) . "%') OR ";
                            } elseif ($filter_scope == 'manufacturer') {
                                $sql_sub .= "LOWER(pm.name) LIKE LOWER('%" . $this->db->escape($words[1]) . "%') OR ";
                            } elseif ($filter_scope == 'upc') {
                                $sql_sub .= "LOWER(p.upc) LIKE LOWER('%" . $this->db->escape($words[1]) . "%') OR ";
                            } elseif ($filter_scope == 'sku') {
                                $sql_sub .= "LOWER(p.sku) LIKE LOWER('%" . $this->db->escape($words[1]) . "%') OR ";
                            } elseif ($filter_scope == 'isbn') {
                                $sql_sub .= "LOWER(p.isbn) LIKE LOWER('%" . $this->db->escape($words[1]) . "%') OR ";
                            } elseif ($filter_scope == 'mpn') {
                                $sql_sub .= "LOWER(p.mpn) LIKE LOWER('%" . $this->db->escape($words[1]) . "%') OR ";
                            } elseif ($filter_scope == 'location') {
                                $sql_sub .= "LOWER(p.location) LIKE LOWER('%" . $this->db->escape($words[1]) . "%') OR ";
                            }
                        }
                        $sql_sub = rtrim($sql_sub, 'OR ');
                        $sql_sub .= ')';
                    } else {
                        $strictAnd = false;
                    }
                ...
                if (!$strictAnd) {
                    ...
                    if (isset($words[0]) && !empty($words[0])) {
                        if (isset($words[1]) && !empty($words[1])) {
                            ...
                            $words_inner = explode(' ', trim(preg_replace('/\s+/', ' ', $words[1])));
                            foreach ($words_inner as $word_inner) {
                                $implode[] = "pd.name LIKE '%" . $this->db->escape($word_inner) . "%'";
                            }
                            if ($implode) {
            ...
            foreach ($data['filter_scopes'] as $filter_scope) {
                if ($filter_scope == 'model') {
                    if ($strictEqual) {
                        $sql .= " OR LOWER(p.model) = LOWER('" . $this->db->escape($words[1]) . "')";
                    } elseif ($strictAnd) {
                        $sql .= " OR LOWER(p.model) LIKE LOWER('%" . $this->db->escape($words[0]) . "%')";
                        $sql .= $sql_sub;
                    } elseif ($strictOr) {
                        $sql .= " OR LOWER(p.model) LIKE LOWER('%" . $this->db->escape($words[0]) . "%')";
                        $sql .= " OR LOWER(p.model) LIKE LOWER('%" . $this->db->escape($words[1]) . "%')";
                    } else {
                        $sql .= " OR LOWER(p.model) LIKE LOWER('%" . $this->db->escape($data['filter_name']) . "%')";
                        $orderBy .= " p.model like concat('" . $this->db->escape($data['filter_name']) . "', '%') desc,
                                    ifnull(nullif(instr(p.model, concat(' ', '" . $this->db->escape($data['filter_name']) . "')), 0), 99999),
                                    ifnull(nullif(instr(p.model, '" . $this->db->escape($data['filter_name']) . "'), 0), 99999),
                                    p.model ,";
                    }
                } elseif ($filter_scope == 'manufacturer') {
                    if ($strictEqual) {
                        $sql .= " OR LOWER(pm.name) = LOWER('" . $this->db->escape($words[1]) . "')";
                    } elseif ($strictAnd) {
                        $sql .= " OR LOWER(pm.name) LIKE LOWER('%" . $this->db->escape($words[0]) . "%')";
                        $sql .= $sql_sub;
                    } elseif ($strictOr) {
                        $sql .= " OR LOWER(pm.name) LIKE LOWER('%" . $this->db->escape($words[0]) . "%')";
                        $sql .= " OR LOWER(pm.name) LIKE LOWER('%" . $this->db->escape($words[1]) . "%')";
                    } else {
                        $sql .= " OR LOWER(pm.name) LIKE LOWER('%" . $this->db->escape($data['filter_name']) . "%')";
                        $orderBy .= " pm.name like concat('" . $this->db->escape($data['filter_name']) . "', '%') desc,
                                    ifnull(nullif(instr(pm.name, concat(' ', '" . $this->db->escape($data['filter_name']) . "')), 0), 99999),
                                    ifnull(nullif(instr(pm.name, '" . $this->db->escape($data['filter_name']) . "'), 0), 99999),
                                    pm.name ,";
                    }
                } elseif ($filter_scope == 'upc') {
                    if ($strictEqual) {
                        $sql .= " OR LOWER(p.upc) = LOWER('" . $this->db->escape($words[1]) . "')";
                    } elseif ($strictAnd) {
                        $sql .= " OR LOWER(p.upc) LIKE LOWER('%" . $this->db->escape($words[0]) . "%')";
                        $sql .= $sql_sub;
                    } elseif ($strictOr) {
                        $sql .= " OR LOWER(p.upc) LIKE LOWER('%" . $this->db->escape($words[0]) . "%')";
                        $sql .= " OR LOWER(p.upc) LIKE LOWER('%" . $this->db->escape($words[1]) . "%')";
                    } else {
                        $sql .= " OR LOWER(p.upc) LIKE LOWER('%" . $this->db->escape($data['filter_name']) . "%')";
                        $orderBy .= " p.upc like concat('" . $this->db->escape($data['filter_name']) . "', '%') desc,
                                    ifnull(nullif(instr(p.upc, concat(' ', '" . $this->db->escape($data['filter_name']) . "')), 0), 99999),
                                    ifnull(nullif(instr(p.upc, '" . $this->db->escape($data['filter_name']) . "'), 0), 99999),
                                    p.upc ,";
                    }
                } elseif ($filter_scope == 'sku') {
                    if ($strictEqual) {
                        $sql .= " OR LOWER(p.sku) = LOWER('" . $this->db->escape($words[1]) . "')";
                    } elseif ($strictAnd) {
                        $sql .= " OR LOWER(p.sku) LIKE LOWER('%" . $this->db->escape($words[0]) . "%')";
                        $sql .= $sql_sub;
                    } elseif ($strictOr) {
                        $sql .= " OR LOWER(p.sku) LIKE LOWER('%" . $this->db->escape($words[0]) . "%')";
                        $sql .= " OR LOWER(p.sku) LIKE LOWER('%" . $this->db->escape($words[1]) . "%')";
                    } else {
                        $sql .= " OR LOWER(p.sku) LIKE LOWER('%" . $this->db->escape($data['filter_name']) . "%')";
                        $orderBy .= " p.sku like concat('" . $this->db->escape($data['filter_name']) . "', '%') desc,
                                    ifnull(nullif(instr(p.sku, concat(' ', '" . $this->db->escape($data['filter_name']) . "')), 0), 99999),
                                    ifnull(nullif(instr(p.sku, '" . $this->db->escape($data['filter_name']) . "'), 0), 99999),
                                    p.sku ,";
                    }
                } elseif ($filter_scope == 'isbn') {
                    if ($strictEqual) {
                        $sql .= " OR LOWER(p.isbn) = LOWER('" . $this->db->escape($words[1]) . "')";
                    } elseif ($strictAnd) {
                        $sql .= " OR LOWER(p.isbn) LIKE LOWER('%" . $this->db->escape($words[0]) . "%')";
                        $sql .= $sql_sub;
                    } elseif ($strictOr) {
                        $sql .= " OR LOWER(p.isbn) LIKE LOWER('%" . $this->db->escape($words[0]) . "%')";
                        $sql .= " OR LOWER(p.isbn) LIKE LOWER('%" . $this->db->escape($words[1]) . "%')";
                    } else {
                        $sql .= " OR LOWER(p.isbn) LIKE LOWER('%" . $this->db->escape($data['filter_name']) . "%')";
                        $orderBy .= " p.isbn like concat('" . $this->db->escape($data['filter_name']) . "', '%') desc,
                                    ifnull(nullif(instr(p.isbn, concat(' ', '" . $this->db->escape($data['filter_name']) . "')), 0), 99999),
                                    ifnull(nullif(instr(p.isbn, '" . $this->db->escape($data['filter_name']) . "'), 0), 99999),
                                    p.isbn ,";
                    }
                } elseif ($filter_scope == 'mpn') {
                    if ($strictEqual) {
                        $sql .= " OR LOWER(p.mpn) = LOWER('" . $this->db->escape($words[1]) . "')";
                    } elseif ($strictAnd) {
                        $sql .= " OR LOWER(p.mpn) LIKE LOWER('%" . $this->db->escape($words[0]) . "%')";
                        $sql .= $sql_sub;
                    } elseif ($strictOr) {
                        $sql .= " OR LOWER(p.mpn) LIKE LOWER('%" . $this->db->escape($words[0]) . "%')";
                        $sql .= " OR LOWER(p.mpn) LIKE LOWER('%" . $this->db->escape($words[1]) . "%')";
                    } else {
                        $sql .= " OR LOWER(p.mpn) LIKE LOWER('%" . $this->db->escape($data['filter_name']) . "%')";
                        $orderBy .= " p.mpn like concat('" . $this->db->escape($data['filter_name']) . "', '%') desc,
                                    ifnull(nullif(instr(p.mpn, concat(' ', '" . $this->db->escape($data['filter_name']) . "')), 0), 99999),
                                    ifnull(nullif(instr(p.mpn, '" . $this->db->escape($data['filter_name']) . "'), 0), 99999),
                                    p.mpn ,";
                    }
                } elseif ($filter_scope == 'location') {
                    if ($strictEqual) {
                        $sql .= " OR LOWER(p.location) = LOWER('" . $this->db->escape($words[1]) . "')";
                    } elseif ($strictAnd) {
                        $sql .= " OR LOWER(p.location) LIKE LOWER('%" . $this->db->escape($words[0]) . "%')";
                        $sql .= $sql_sub;
                    } elseif ($strictOr) {
                        $sql .= " OR LOWER(p.location) LIKE LOWER('%" . $this->db->escape($words[0]) . "%')";
                        $sql .= " OR LOWER(p.location) LIKE LOWER('%" . $this->db->escape($words[1]) . "%')";
                    } else {
                        $sql .= " OR LOWER(p.location) LIKE LOWER('%" . $this->db->escape($data['filter_name']) . "%')";
                        $orderBy .= " p.location like concat('" . $this->db->escape($data['filter_name']) . "', '%') desc,
                                    ifnull(nullif(instr(p.location, concat(' ', '" . $this->db->escape($data['filter_name']) . "')), 0), 99999),
                                    ifnull(nullif(instr(p.location, '" . $this->db->escape($data['filter_name']) . "'), 0), 99999),
                                    p.location ";
                    }
                }
            }
    ...
    // add for product based discount end
    public function deleteEmptyPosOrders() {
        $order_query = $this->db->query("SELECT order_id FROM `" . DB_PREFIX . "order` WHERE is_pos = 1  AND total = 0.0000 AND date_added <= NOW() - INTERVAL 1 DAY ORDER BY order_id DESC");
        if (!empty($order_query->rows)) {
            foreach ($order_query->rows as $row) {
                ...
                $this->db->query("DELETE FROM " . DB_PREFIX . "order_fraud WHERE order_id = '" . $row['order_id'] . "'");
                $this->db->query("DELETE FROM " . DB_PREFIX . "customer_transaction WHERE order_id = '" . $row['order_id'] . "'");
                $this->db->query("DELETE FROM " . DB_PREFIX . "customer_reward WHERE order_id = '" . $row['order_id'] . "'");
                $this->db->query("DELETE FROM " . DB_PREFIX . "affiliate_transaction WHERE order_id = '" . $row['order_id'] . "'");
                $this->db->query("DELETE FROM " . DB_PREFIX . "order_payment WHERE order_id = '" . $row['order_id'] . "'");
            }
        }
    }
    ...
    public function getCustomers($data = array()) {
        ...
        if (!empty($data['filter_name'])) {
            $sql .= " OR company_id LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
        }
    ...
    public function editCustomer($customer_id, $data) {
        ...
        if (isset($data['addresses'])) {
            foreach ($data['addresses'] as $address) {
                ...
                if (isset($data['customer_sales_notes'])) {

                    $this->db->query("DELETE FROM " . DB_PREFIX . "customer_notes WHERE customer_id='" . $customer_id . "' ");
                    $this->db->query("INSERT INTO " . DB_PREFIX . "customer_notes SET customer_notes='" . $this->db->escape($data['customer_sales_notes']) . "',customer_image='" . $this->db->escape($data['customer_image']) . "',customer_id='" . $customer_id . "' ");
                }
            }
        }
    }
    ...
    public function getCustomerGroups($data = array()) {
        $sql = "SELECT * FROM " . DB_PREFIX . "customer_group cg LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (cg.customer_group_id = cgd.customer_group_id) WHERE cgd.language_id = '" . (int) $this->config->get('config_language_id') . "' ORDER BY cg.customer_group_id ASC";
    ...
    public function getCustomerByEmail($customer_email) {
        $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer WHERE email = '" . $customer_email . "'");
    ...
    public function getDiscountPercent($discount_quantity = null, $product_id, $customer_group_id) {
     ...
     * @return type
     */
    public function geProductCalculatedPrice($product_id, $quantity = 1, $unit_mult = 1, $customer_group_id, $option_details = null) {
        $base_price = $this->getMainPrice($product_id);

        if (!empty($unit_mult) && !is_null($unit_mult)) {
            $discount_qty = $quantity * $unit_mult;
            $discount_multiplier = $this->getDiscountPercent($discount_qty, $product_id, $customer_group_id);
            $final_price = $discount_multiplier * $base_price * $unit_mult;
        } else {
            $discount_multiplier = $this->getDiscountPercent($quantity, $product_id, $customer_group_id);
    ...
    public function getUnitConvertedValue($product_id, $unit_conversion_product_id) {
        $query = $this->db->query("SELECT convert_price FROM " . DB_PREFIX . "unit_conversion_product WHERE product_id = '" . (int) $product_id . "' AND unit_conversion_product_id = '" . (int) $unit_conversion_product_id . "'");

        return $query->row ? $query->row['convert_price'] : 1;
    }

    public function getProductUnitConversionData($product_id) {
        $query = $this->db->query("SELECT ucp.*, ucv.name FROM " . DB_PREFIX . "unit_conversion_product ucp INNER JOIN " . DB_PREFIX . "unit_conversion_value ucv ON ucp.unit_value_id = ucv.unit_value_id  WHERE ucp.product_id = '" . (int) $product_id . "'");

        return $query->rows;
    }

    public function getSelectedOrderProductUnit($order_product_id) {
        $query = $this->db->query("SELECT unit_conversion_values FROM " . DB_PREFIX . "order_product WHERE order_product_id = '" . (int) $order_product_id . "'");
        return $query->row ? $query->row['unit_conversion_values'] : 0;
    }

    public function getProductDiscountedPrice($product_id, $unit_conversion_values = 0, $customer_group_id = 1, $quantity = 1, $price) {

        if ($quantity == NULL) {
            $quantity = 1;
        }
        if ($unit_conversion_values != 0) {
            $query = $this->db->query("SELECT convert_price FROM " . DB_PREFIX . "unit_conversion_product WHERE product_id = '" . (int) $product_id . "' AND unit_conversion_product_id = '" . (int) $unit_conversion_values . "'");
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_discount WHERE product_id = '" . (int) $product_id . "' AND customer_group_id = '" . (int) $customer_group_id . "' AND quantity <= '" . $discounted_quantity . "' ORDER BY quantity DESC LIMIT 1");
    ...
    public function saveResaleIdNumber($order_id, $resale_text) {
        $this->db->query("UPDATE `" . DB_PREFIX . "order` SET resale_text = '" . $this->db->escape($resale_text) . "' WHERE order_id = '" . (int) $order_id . "'");
    }

    public function getWaitlist($limit, $offset) {
        $query = $this->db->query("SELECT w.*, ws.name AS status FROM (SELECT * FROM " . DB_PREFIX . "waitlist LIMIT " . $offset . "," . $limit . ") w LEFT JOIN " . DB_PREFIX . "waitlist_status AS ws ON w.waitlist_status_id = ws.waitlist_status_id");
        return $query->rows;
    }

    public function getPosWaitlist($data, $limit, $offset) {
        $sql = "SELECT w.*, ws.color as status_color,ws.name AS status FROM (SELECT * FROM " . DB_PREFIX . "waitlist LIMIT " . $offset . "," . $limit . ") w LEFT JOIN " . DB_PREFIX . "waitlist_status AS ws ON w.waitlist_status_id = ws.waitlist_status_id WHERE 1 ";
        if ($data) {
            if (isset($data['name']))
                $sql .= ' AND  w.name like "%' . $data['name'] . '%"';
            if (isset($data['ticket_number']))
                $sql .= ' AND w.ticket_number like "%' . $data['ticket_number'] . '%"';
            if (isset($data['waitlist_status_id']))
                $sql .= ' AND w.waitlist_status_id = ' . $data['waitlist_status_id'];
            if (isset($data['filter_date_added']))
                $sql .= ' AND date(w.date_added) = "' . $data['filter_date_added'] . '"';
        }
        $query = $this->db->query($sql);
        return $query->rows;
    }

    public function getTotalWaitlist($data) {
        $sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "waitlist WHERE 1";
        if ($data) {
            if (isset($data['name']))
                $sql .= ' AND  name like "%' . $data['name'] . '%"';
            if (isset($data['ticket_number']))
                $sql .= ' AND ticket_number like "%' . $data['ticket_number'] . '%"';
            if (isset($data['waitlist_status_id']))
                $sql .= ' AND waitlist_status_id = ' . $data['waitlist_status_id'];
            if (isset($data['filter_date_added']))
                $sql .= ' AND date(date_added) = "' . $data['filter_date_added'] . '"';
        }
    ...
    public function getWaitlistStatus() {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "waitlist_status");
        return $query->rows;
    }

    public function updateWaitlistStatus($id, $status) {
        return $this->db->query("UPDATE " . DB_PREFIX . "waitlist set waitlist_status_id=" . (int) $status . " where waitlist_id =" . (int) $id);
    }
