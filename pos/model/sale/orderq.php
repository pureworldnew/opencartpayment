...
class ModelSaleOrderq extends Model {
	...
	public function createBackOrder($request, $product_id, $customer_id, $order_id, $quantity, $address_id = 0)
	{   
		...
		if( $customer_id > 0 )
		{
			...
			$customer_id = $this->getCustomerByOrderID($order_id);
			$customer_info = $this->getCustomerByID($customer_id); 
		}
		if (empty($customer_info)) {
			//load default customer
			$customer_info = $this->getCustomerByEmail('customer@instore.com');
		}
		...
		} else if( $customer_info['address_id'] > 0 )
		{
			$address_info = $this->getAddress($customer_info['address_id']);
		} else {  
			$address_info = $this->getAddressByCustomerID($customer_info['customer_id']);
		}
		
		$address_company = isset($address_info['company']) ? $address_info['company'] : "";
		$address_1 = isset($address_info['address_1']) ? $address_info['address_1'] : "";
		$address_2 = isset($address_info['address_2']) ? $address_info['address_2'] : "";
		$address_city = isset($address_info['city']) ? $address_info['city'] : "";
		$address_postcode = isset($address_info['postcode']) ? $address_info['postcode'] : "";
		$address_country = isset($address_info['country']) ? $address_info['country'] : "";
		$address_country_id = isset($address_info['country_id']) ? $address_info['country_id'] : "";
		$address_zone = isset($address_info['zone']) ? $address_info['zone'] : "";
		$address_zone_id = isset($address_info['zone_id']) ? $address_info['zone_id'] : "";


		$this->db->query("INSERT INTO `" . DB_PREFIX . "order` SET invoice_prefix = '', store_id = '".$customer_info['store_id']."', store_name = 'Gempacked', store_url = '".$store_url."', customer_id = '".$customer_info['customer_id']."', customer_group_id = '".$customer_info['customer_group_id']."', firstname = '".$customer_info['firstname']."', lastname = '".$customer_info['lastname']."', email = '".$customer_info['email']."', telephone = '".$customer_info['telephone']."', fax = '".$customer_info['fax']."', custom_field = '', payment_firstname = '".$customer_info['firstname']."', payment_lastname = '".$customer_info['lastname']."', payment_company = '".$address_company."', payment_address_1 = '".$address_1."', payment_address_2 = '".$address_2."', payment_city = '".$address_city."', payment_postcode = '".$address_postcode."', payment_country = '".$address_country."', payment_country_id = '".$address_country_id."', payment_zone = '".$address_zone."', payment_zone_id = '".$address_zone_id."', payment_address_format = '', payment_custom_field = '', payment_method = 'Bill Me Later', payment_code = 'cheque', shipping_firstname = '".$customer_info['firstname']."', shipping_lastname = '".$customer_info['lastname']."', shipping_company = '".$address_company."', shipping_address_1 = '".$address_1."', shipping_address_2 = '".$address_2."', shipping_city = '".$address_city."', shipping_postcode = '".$address_postcode."', shipping_country = '".$address_country."', shipping_country_id = '".$address_country_id."', shipping_zone = '".$address_zone."', shipping_zone_id = '".$address_zone_id."', shipping_address_format = '', shipping_custom_field = '', shipping_method = 'In-Store Pick up', shipping_code = 'xshippingpro.xshippingpro2', comment = '', total = '', affiliate_id = '',order_status_id='1', commission = '', marketing_id = '', tracking = '', language_id = '', currency_id = '',currency_value='1', currency_code = 'USD', order_type='3', manufacturer_id = '0', date_added = NOW(), date_modified = NOW()");

		$backorder_id = $this->db->getLastId();

		$product_info  = $this->getProductData($product_id);
  
		if( $product_info )
		{
			$total = $product_info['price'] * $quantity;
			$this->db->query("INSERT INTO ".DB_PREFIX."order_product SET order_id='".$backorder_id."',product_id='".$product_id."' ,name='".$this->db->escape($product_info['name'])."',model='".$this->db->escape($product_info['model'])."',price='".$product_info['price']."',quantity='".$quantity."',default_vendor_unit='".$this->db->escape($product_info['default_vendor_unit'])."',total='".$total."'");
			...
			$message = "Backorder created for " . $customer_name . "! " . $quantity . " " . $product_info['unit_plural'] . " of " . $product_info['model'];
			return $message; 
		}
	...
	public function getAddressByCustomerID($customer_id) {
		$address_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "address WHERE customer_id = '" . (int)$customer_id . "' LIMIT 1");
	...
	public function pendingIncomingOrder( $manufacturer_id )
	{
		$query = $this->db->query("SELECT order_id FROM ". DB_PREFIX ."order WHERE manufacturer_id	 = '". $manufacturer_id ."' AND order_type = 2 AND order_status_id = 1 ORDER BY order_id DESC LIMIT 1");

		$order_id = $query->row ? $query->row['order_id'] : 0;
		return $order_id;
	}

	public function addFirstProductToIncomingOrder( $order_id, $product_id, $previous_order_id, $quantity )
	{
		$quantity = floatval($quantity);
	...
	public function addProductToIncomingOrder( $order_id, $product_id, $previous_order_id, $quantity )
	{
		$quantity = floatval($quantity);
		...
		if( $product_data )
		{
			if( $product_exist )
			{
				$this->db->query("UPDATE " . DB_PREFIX . "order_product SET  quantity = quantity + '".$quantity."' WHERE order_id ='".$order_id."' AND product_id='".$product_id."'");
