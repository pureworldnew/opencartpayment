...
<div id="content">
  <div class="page-header">
    <div class="container-fluid">
      <div class="pull-right"><button onclick="window.top.close();" class="btn btn-default"><i class="fa fa-reply"></i> <?php echo $button_back; ?></button></div>
                  ...
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                     <div id="maindiv">
                      ...
                      <p> Only JPEG,PNG,JPG Type Image Uploaded.</p>
                    ...
                    <td>
                      <select name="payment_address" id="input-payment-address" class="form-control">
                          ...
                          <?php foreach ($addresses as $address) { ?>
                          	...
                          	<?php }else{?>
                            		<option selected="selected" value="<?php echo $address['address_id']; ?>"><?php echo $address['firstname'] . ' ' . $address['lastname'] . ', ' . $address['address_1'] . ', ' . $address['city'] . ', '. $address['postcode'] . ', ' . $address['country']; ?></option>
...
$('#button-refresh').on('click', function() {
	$.ajax({
		url: $('select[name=\'store\'] option:selected').val() + 'index.php?route=api/orderquotecart/products&token=' + token2,
		dataType: 'json',
		crossDomain: true,
		success: function(json) {
          ...
          html += '  <td class="text-left"><img src="' + product['image'] + '" alt="' + product['name'] + '" title="' + product['name'] + '"></td>';
					...
					if(product['unit']['unit_conversion_values']) {
						...
						html += '  <input type="hidden" name="product[' + i + '][quantity]" value="' +  product['quantity'] + '" />';
          			...
          			html += '  <td class="text-right"><input class="form-control productprice" type="text" value="'+product['newprice']+'" name="product[' + i + '][price]" /><!--' + product['pricewithtax'] + '<br><small>Ex.Tax ' + product['price'] + '<small>-->';
...
function gettotals() {
  $.ajax({
    url: $('select[name=\'store\'] option:selected').val() + 'index.php?route=api/orderquotecart/total&token=' + token2,
    dataType: 'json',
    crossDomain: true,
    success: function(json) {
...
$('#addproducts input[name=\'product\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=module/pos/autocomplete_product_pos&backend=1&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request)+ '&filter_model2=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
...
$('#button-save').on('click', function() {
	...
	$.ajax({
      ...
      $('#resume_order', window.parent.document).hide();
      $('#button_print_receipt_edit_order', window.parent.document).show();
