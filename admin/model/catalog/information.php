...
class ModelCatalogInformation extends Model {
    ...
    public function getInformationsWithURL($keyword){
     	$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "information i LEFT JOIN " . DB_PREFIX . "information_description id ON (i.information_id = id.information_id) INNER JOIN ".DB_PREFIX."url_alias ua ON (concat('information_id=',i.information_id) = ua.query) WHERE id.language_id = '" . (int)$this->config->get('config_language_id') . "' AND ua.keyword like '%".$keyword."%'");
         return $query->rows;
    }
}