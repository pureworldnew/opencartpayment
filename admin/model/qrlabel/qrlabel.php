...
class ModelQrlabelQrlabel extends Model {
	...
	public function generateQRCodesGeneral($data = array(),$sets = array()) {
		require_once(DIR_SYSTEM.'/library/'."phpqrcode.php");
		$dir = 'qrcodes/qrcodes-'.date("Y-m-d-H:i")."/";
		$TEMP_DIR = DIR_IMAGE.$dir;
		$files = array();
		...
		foreach ($data as $product){
			$filename = $TEMP_DIR.substr(md5($product['title']),0,7).'.png';
			$qrcode_string = "";
			$qrcode_string = $product['qr_text'];
			QRcode::png($qrcode_string, $filename, 'L', 4, 2);
			$files['qrFiles'][] = $filename;
		}
		return $files;
	...
	public function generateReportGeneral($filters = array(),$sets = array(),$custom_data = array()) {
		...
		if (!empty($products)) {
			...
			$pdf->SetTitle('Non Model Products QR Codes');
			$pdf->SetSubject('QR CODES');
			$pdf->SetKeywords('QR,GEMPACK');
			
			// set default header data
			$pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE.' QR CODES FOR NON-MODEL PRODUCTS', PDF_HEADER_STRING);
			...
			foreach($products as $product) {
				...
				$pdf->write2DBarcode($text, 'QRCODE,Q', $startx, $starty, 50, 50, $style, 'N');
				$pdf->Text($startx, $starty - 5, $title);
	...
	public function impCSVFileWithOutModel($file,$ispdf=false) {
		$args = array();
		//$this->bgprocess->run(__FILE__,__CLASS__,"processImport",$args);
		$file_path = __FILE__;
		...
		if ($file) {
			...
			if (count($data) > 1) {
				...
				for ($j = 0 ; $j < count($data) ; $j++) {
					...
					if ($_name && $_data){
						$log->write("writing for col=$_name,value=$_data");
					}else{
						$log->write("Error for col = $_name,value=$_data - Not exists");
					}
				}
				
				$product['qr_text'] = $_format;
				$product['title'] = $_text;
				$log->write("Get data for Item = $_text");
				$log->write(json_encode($product));
				$products[] = $product;
		...
		$sets['filter_text'] = '';
		$sets['default_format'] = null;
		$sets['gen_name'] = date("Y-m-d-h-i").rand().'no_model';
		$sets['download'] = true;
		
		$resp = array();
		if ($ispdf === "true") {
			$resFile = $this->generateReportGeneral(array(),$sets,$products);
			
			$resp['mode'] = 'pdf';
			$resp['code'] = 200;
			$resp['file'] = $this->getDownloadURL($resFile);
		}else{
			$list = $this->generateQRCodesGeneral($products,$sets);
			$resFile = $this->generateZip($list['baseDir'],$list['qrFiles'],"qrcodes.zip");
