...
class ModelPosSeoUrl extends Model {
    ...
    public function checkIfGrouped($product_id) {
        ...
        $exist_check_query = $this->db->query("SELECT groupindicator_id FROM product_concat_temp_table WHERE product_id=" . (int)$product_id . "");
    ...
    public function isProductGrouped($product_id) {
        ...
        if ( !empty($query->row) && $query->row['model'] == 'grouped' ) {
    ...
    public function getGroupProductId($product_id) {
        $query = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product_grouped WHERE grouped_id=" . (int)$product_id . "");
        if ($query->num_rows) {
    ...
    public function getproductIdByProductId($product_id) {
        $sel_query = $this->db->query("SELECT groupindicator_id from product_concat_temp_table WHERE product_id = '" . (int)$product_id . "'");
        if ($sel_query->num_rows) {
            return $this->getGroupProductIds($sel_query->row['groupindicator_id']);
        } else {
            return FALSE;
