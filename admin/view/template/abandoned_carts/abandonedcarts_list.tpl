          ...
          <div class="tab-content">
            <div class="tab-pane active" id="tab-abandoned-carts">
              <div class="well well-sm">
							...
							<?php if($abandoned_templates && $abandoned_carts_email_send_status) { ?>
							<div class="well well-sm">
								<div class="row">
									<div class="col-sm-3">
										<select class="form-control" data-toggle="tooltip" title="<?php echo $help_mail_template; ?>" name="abandoned_template_id">
											<?php foreach($abandoned_templates as $abandoned_template) { ?>
											<option value="<?php echo $abandoned_template['abandoned_template_id']; ?>"><?php echo $abandoned_template['title']; ?></option>
											<?php } ?>
										</select>
									</div>
									<div class="col-sm-3">
										<button type="button" id="button-send" disabled class="btn btn-primary btn-block"><i class="fa fa-share"></i> <?php echo $button_send; ?></button>
									</div>
								</div>
							</div>
							<?php } ?>								
...
$('#button-filter').on('click', function() {
	var url = 'index.php?route=abandoned_carts/abandonedcarts/getCarts&token=<?php echo $token; ?>';
...
//--></script>
<script type="text/javascript"><!--
$(document).ready(function(){	$('.abandoned_carts_class').addClass('active'); });
$(document).ready(function(){	$('#button-filter').trigger('click'); });
//--></script>
<script type="text/javascript"><!--
$(document).delegate('.peoples .selected_data, .peoples .allcheckbox', 'click',function() {
	
	// hidden active
	$('.peoples .selected_data').each(function () {
		if (this.checked) {
			$(this).parent().addClass('hidden_active');
		}else{
			$(this).parent().removeClass('hidden_active');
		}
	});
	...
	var checkedsel = 0;
	$('.peoples .selected_data').each(function () {
		 checkedsel = $('.peoples .selected_data:checked').length;
	});
...
$('#button-send').click(function() {
	var abandoned_template_id = $('select[name=\'abandoned_template_id\']').val();
	
	var url = 'index.php?route=abandoned_carts/abandonedcarts/send&token=<?php echo $token; ?>&abandoned_template_id=' + abandoned_template_id;
		
	if(abandoned_template_id) {
		var filter_name = $('input[name=\'filter_name\']').val();
		...
		$.ajax({
			...
			dataType: 'json',
			type: 'post',
			data: $('input[type=\'checkbox\']:checked, .hidden_active input[type=\'hidden\']'),
			beforeSend: function() {
