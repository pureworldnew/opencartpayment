      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-setting" class="form-horizontal">
          <ul class="nav nav-tabs">
            ...
            <li><a href="#tab-estimate" data-toggle="tab"><?php echo 'Estimate Delivery Time'; ?></a></li>
            <li><a href="#tab-usergroup" data-toggle="tab"><?php echo 'Asign Users Groups'; ?></a></li>
          </ul>
          <div class="tab-content">
            <div class="tab-pane active" id="tab-general">
			  <div class="form-group">
                  <label class="col-sm-2 control-label" for="input-meta-wires">Additional percentage for Wires</label>
                  <div class="col-sm-10">
                    <input type="text" name="config_additional_percentage" value="<?php echo $config_additional_percentage; ?>" placeholder="Additional percentage for Wires" id="input-meta-wires" class="form-control" />
            ...
            <div class="tab-pane" id="tab-usergroup">
            <?php //echo '<pre>';print_r($user_groups);
            	if(!empty($user_groups)){
            ?>
            	<!--<label class="col-sm-2 control-label">
            		<span data-toggle="tooltip" title="" data-original-title="Select users groups which you want to show in orders filter">Select users groups</span>
            	</label>
            	<div class="col-sm-10">
			    	<select name="config_users_group" class="form-control">
			    		<?php foreach($user_groups as $getugroups){
			    			echo '<option value="'.$getugroups['user_group_id'].'" '.($config_users_group == $getugroups['user_group_id'] ? 'selected=selected':'').'>'.$getugroups['name'].'</option>';
			    		}?>
			    	</select>
            	</div>-->
                <table id="estimatedays" class="table table-striped table-bordered table-hover">
                	...
                	<?php foreach($user_groups as $getugroups){ 	
                		echo '<tr>
                			<td width="30">
                				<input type="checkbox" name="config_users_group[]" value="'.$getugroups['user_group_id'].'" '.(in_array((int)$getugroups['user_group_id'],$config_users_group) ? "checked":"").'>
                			</td>
                			<td>'.$getugroups['name'].'</td>
                		</tr>';
					}?>	
                	</tbody>
            	</table>
        	...
        	<div class="tab-pane" id="tab-estimate">
                <table id="estimatedays" class="table table-striped table-bordered table-hover">
                  ...
                  <tfoot>
                    <tr>
                      <td colspan="2"></td>
                      <td class="text-left"><button type="button" onclick="addEstimatedays();" data-toggle="tooltip" title="<?php echo 'Add Esitmate Days'; ?>" class="btn btn-primary"><i class="fa fa-plus-circle"></i></button></td>
                    </tr>
                  </tfoot>
                </table>
...
function addEstimatedays() {
  ...
  html += '<div class="input-group"><textarea name="estimatedays[' + estimatedays_row + '][estimatedays_description]" rows="5" placeholder="Description" class="form-control"></textarea></div>';
  html += '  </td>';
  html += '  <td class="text-left"><button type="button" onclick="$(\'#estimatedays-row' + estimatedays_row + '\').remove();" data-toggle="tooltip" title="Remove" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>';
  html += '</tr>';
