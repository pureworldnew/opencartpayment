...
class ControllerCommonHeader extends Controller {
	public function index() {
		...
		$this->db->query("DELETE FROM ".DB_PREFIX."cart WHERE session_id='".$this->session->getId()."' AND is_admin='1'");
		...
		if (!isset($this->request->get['token']) || !isset($this->session->data['token']) || ($this->request->get['token'] != $this->session->data['token'])) {
			...
			// Processing Orders
			$data['processing_status_total'] = 0;//$this->model_sale_order->getTotalOrders(array('filter_order_status' => implode(',', $this->config->get('config_processing_status'))));
		...
		//	$this->load->model('sale/return');
			...
			$return_total = 0;//$this->model_sale_return->getTotalReturns(array('filter_return_status_id' => $this->config->get('config_return_status_id')));
...
//			$this->load->model('customer/customer');
			...
			$customer_total = 0;//$this->model_customer_customer->getTotalCustomers(array('filter_approved' => false));
...
//			$this->load->model('catalog/review');
			...
			$review_total = 0;//$this->model_catalog_review->getTotalReviews(array('filter_status' => false));
...
//			$this->load->model('marketing/affiliate');
			...
			$affiliate_total = 0;//$this->model_marketing_affiliate->getTotalAffiliates(array('filter_approved' => false));
			...
			$results = array();//$this->model_setting_store->getStores();
