<?php
class ControllerSaleBackorder extends Controller {
	private $error = array();
	public function index() {
		$this->load->language('sale/backorder');
	...
	public function editbackorder_row(){
		if(isset($this->request->get['product_id'])){
			$this->db->query("DELETE FROM ".DB_PREFIX."order_product WHERE product_id='".$this->request->get['product_id']."' AND order_id='".$this->request->get['order_id']."'");
             $this->db->query("UPDATE ".DB_PREFIX."product SET backorder_request='0' WHERE product_id='".$this->request->get['product_id']."' ");
	...
	public function uploadOrderImages(){
		if (isset($this->request->post['submit'])) {
			...
			if(!empty($this->request->post['order_id']))
			{
				//$this->response->redirect($this->url->link('sale/backorder/edit&order_id='.$order_id, 'token=' . $this->session->data['token'], 'SSL'));
			}
			
			$this->response->redirect($this->url->link('sale/backorder', 'token=' . $this->session->data['token'], 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sale/backorder', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		$data['payment_methods'] = $this->getEnabledPaymentMethods();
		$data['sales_persons'] = $this->model_sale_backorder->getSalesPersons($this->config->get('config_users_group'));
		$data['invoice'] = $this->url->link('sale/backorder/invoice', 'token=' . $this->session->data['token'], 'SSL');
		$data['shipping'] = $this->url->link('sale/backorder/shipping', 'token=' . $this->session->data['token'], 'SSL');
		$data['add'] = $this->url->link('sale/orderq/add', 'token=' . $this->session->data['token'] .'&order_type=backorder', 'SSL');
		$data['orders'] = array();
		$filter_data = array(
		...
		$order_total = $this->model_sale_backorder->getTotalOrders($filter_data);
		$results = $this->model_sale_backorder->getOrders($filter_data);
		...
		foreach ($products as $product) {
			...
			$data['orders'][] = array(
				...
				'sales_person' 			  => $this->model_sale_backorder->getSalesPerson($result['order_id']),
				'histories'		 		  => $histories,
				'order_has_image' 		  => $this->model_sale_backorder->orderHasImage($result['order_id']),
				'status'        		  => $result['status'],
				'no_of_items'             => $no_of_items,
				'model'           		  => $no_of_items > 1 ? $no_of_items . " Items" : $order_products[0]['model'],
				'manufacturers'           => $this->getOrderedProductsManufacturers($result['order_id']),
				'quantity'        	      => $this->getOrderedProductsQuantities($result['order_id']),
				...
				'shipping_code' 		  => $result['shipping_code'],
				'view'          		  => $this->url->link('sale/backorder/info', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL'),
				'copy'          		  => $this->url->link('sale/backorder/copy', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL'),
				'pdfinv_packing'       	  => $this->url->link('sale/order/pdf_packingslip', 'token=' . $this->session->data['token'] . '&type=backorder&order_id=' . $result['order_id'] . $url, 'SSL'),
				'link_pdfinv_invoice'     => $this->url->link('sale/order/pdf_invoice', 'token=' . $this->session->data['token'] . '&type=backorder&order_id=' . $result['order_id'] . $url, 'SSL'),
       ...
       if (isset($this->request->get['filter_manufacturer'])) {
		...
		$data['sort_order'] = $this->url->link('sale/backorder', 'token=' . $this->session->data['token'] . '&sort=o.order_id' . $url, 'SSL');
		$data['sort_customer'] = $this->url->link('sale/backorder', 'token=' . $this->session->data['token'] . '&sort=customer' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('sale/backorder', 'token=' . $this->session->data['token'] . '&sort=status' . $url, 'SSL');
		$data['sort_total'] = $this->url->link('sale/backorder', 'token=' . $this->session->data['token'] . '&sort=o.total' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('sale/backorder', 'token=' . $this->session->data['token'] . '&sort=o.date_added' . $url, 'SSL');
		$data['sort_date_modified'] = $this->url->link('sale/backorder', 'token=' . $this->session->data['token'] . '&sort=o.date_modified' . $url, 'SSL');
		$data['sort_payment_method'] = $this->url->link('sale/backorder', 'token=' . $this->session->data['token'] . '&sort=o.payment_method' . $url, 'SSL');
		$data['sort_shipping_method'] = $this->url->link('sale/backorder', 'token=' . $this->session->data['token'] . '&sort=o.shipping_method' . $url, 'SSL');
		$url = '';
		if (isset($this->request->get['filter_order_id'])) {
			$url .= '&filter_order_id=' . $this->request->get['filter_order_id'];
		}
		...
		$pagination->url = $this->url->link('sale/backorder', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('sale/backorder_list.tpl', $data));
	}
	...
	public function backorder_summary() {
		...
		$backorder_statuses = $this->config->get('selected_backorder_statuses');
		$filter_data = array(
			'order'						=> 'DESC',
		...
		foreach ($results as $result) {
			...
			$data['backorders'][] = array(
				...
				'status'        		  => $result['status'],
				'no_of_items'             => $no_of_items,
				'model'           		  => $no_of_items > 1 ? $no_of_items . " Items" : $products[0]['model'],
				'manufacturers'           => $this->getOrderedProductsManufacturers($result['order_id']),
				'quantity'        	      => $this->getOrderedProductsQuantities($result['order_id']),
				'sales_person' 			  => $this->model_sale_backorder->getSalesPerson($result['order_id']),
				'total'         		  => $this->currency->format($result['total'], $result['currency_code'], $result['currency_value']),
				...
				'date_modified' 		  => date($this->language->get('date_format_short'), strtotime($result['date_modified'])),
				'order_status_color'      => $this->model_localisation_order_status->getOrderStatuscolor($result['order_status_id']),
				'view'          		  => $this->url->link('sale/backorder/info', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'], 'SSL'),
				'copy'          		  => $this->url->link('sale/backorder/copy', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'], 'SSL'),
				'pdfinv_packing'       	  => $this->url->link('sale/order/pdf_packingslip', 'token=' . $this->session->data['token'] . '&type=backorder&order_id=' . $result['order_id'], 'SSL'),
				'link_pdfinv_invoice'     => $this->url->link('sale/order/pdf_invoice', 'token=' . $this->session->data['token'] . '&type=backorder&order_id=' . $result['order_id'], 'SSL'),
				'link_pdf_order_request'  => $this->url->link('sale/order/pdf_order_request', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'], 'SSL'),
				'edit'          		  => $this->url->link('sale/orderq/edit', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'], 'SSL')
			);
		}
		$backorder_total = $this->model_sale_backorder->getTotalOrders($filter_data);
		$pagination = new Pagination();
		$pagination->total = $backorder_total;
		$pagination->page = $page;
		$pagination->limit = 5;
		if( isset($this->request->get['order_id']) )
		{
			$pagination->url = $this->url->link('sale/backorder/backorder_summary', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'] . '&page={page}', 'SSL');
		} else {
			$pagination->url = $this->url->link('sale/backorder/backorder_summary', 'token=' . $this->session->data['token'] . '&page={page}', 'SSL');
		}
		$data['pagination'] = $pagination->render();
		$data['results'] = sprintf($this->language->get('text_pagination'), ($backorder_total) ? (($page - 1) * 5) + 1 : 0, ((($page - 1) * 5) > ($backorder_total - 5)) ? $backorder_total : ((($page - 1) * 5) + 5), $backorder_total, ceil($backorder_total / 5));
		$this->response->setOutput($this->load->view('sale/backorder_summary.tpl', $data));
	}

	public function getOrderedProductsQuantities($order_id){

		$query=$this->db->query("SELECT sum(quantity) as total FROM " . DB_PREFIX . "order_product WHERE order_id='".$order_id."'");
	...
	public function getForm() {
		...
		$data['cancel'] = $this->url->link('sale/backorder', 'token=' . $this->session->data['token'] . $url, 'SSL');
		if (isset($this->request->get['order_id'])) {
			$order_info = $this->model_sale_backorder->getOrder($this->request->get['order_id']);
		}
		if (!empty($order_info)) {
			...
			$products = $this->model_sale_backorder->getOrderProducts($this->request->get['order_id']);
			foreach ($products as $product) {
				$data['order_products'][] = array(
					'product_id' => $product['product_id'],
					'name'       => $product['name'],
					...
					'option'     => $this->model_sale_backorder->getOrderOptions($this->request->get['order_id'], $product['order_product_id']),
					'quantity'   => $product['quantity'],
					'price'      => $product['price'],
					'total'      => $product['total'],
			...
			// Vouchers
			$data['order_vouchers'] = $this->model_sale_backorder->getOrderVouchers($this->request->get['order_id']);
			$data['coupon'] = '';
			$data['voucher'] = '';
			$data['reward'] = '';
			$data['order_totals'] = array();
			$order_totals = $this->model_sale_backorder->getOrderTotals($this->request->get['order_id']);
	...
	public function copy()
	{
		...
		$new_order_id = $this->model_sale_backorder->copyOrder($order_id);
		if($new_order_id)
		{
	...
	public function info() {
		...
		$order_info = $this->model_sale_backorder->getOrder($order_id);
		if ($order_info) {
			$this->load->language('sale/backorder');
			$this->document->setTitle($this->language->get('heading_title'));
			$data['heading_title'] = $this->language->get('heading_title');
			...
			$data['shipping'] = $this->url->link('sale/backorder/shipping', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
			$data['invoice'] = $this->url->link('sale/backorder/invoice', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
			$data['edit'] = $this->url->link('sale/backorderq/edit', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
			$data['cancel'] = $this->url->link('sale/backorder', 'token=' . $this->session->data['token'] . $url, 'SSL');
			$data['order_id'] = $this->request->get['order_id'];
			...
			//exit;
			$data['sales_persons'] = $this->model_sale_backorder->getSalesPersons($this->config->get('config_users_group'));
			$data['sales_person'] = $this->model_sale_backorder->getSalesPerson($data['order_id']);
			...
			foreach ($products as $product) {
				$option_data = array();
				$options = $this->model_sale_backorder->getOrderOptions($this->request->get['order_id'], $product['order_product_id']);
				foreach ($options as $option) {
					if ($option['type'] != 'file') {
						$option_data[] = array(
							'name'  => $option['name'],
				...
				$data['products'][] = array(
					...
					'href'     		   => $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $product['product_id'], 'SSL'),
					'unitdatanames' => $this->model_sale_backorder->getunitdataname($product['product_id']),
					'getDefaultUnitDetails' => $this->model_sale_backorder->getDefaultUnitDetails($product['product_id'])
				);
			}
			$data['vouchers'] = array();
			$vouchers = $this->model_sale_backorder->getOrderVouchers($this->request->get['order_id']);
			...
			$data['totals'] = array();
			$totals = $this->model_sale_backorder->getOrderTotals($this->request->get['order_id']);
			...
			$data['column_left'] = $this->load->controller('common/column_left');
			$data['footer'] = $this->load->controller('common/footer');
			$this->response->setOutput($this->load->view('sale/backorder_info.tpl', $data));
		} else {
	...
	public function createInvoiceNo() {
		$this->load->language('sale/backorder');
		$json = array();
		if (!$this->user->hasPermission('modify', 'sale/backorder')) {
			...
			$this->load->model('sale/backorder');
			$invoice_no = $this->model_sale_backorder->createInvoiceNo($order_id);
			if ($invoice_no) {
	...
	public function history() {
		...
		$this->load->model('sale/backorder');
		$results = $this->model_sale_backorder->getOrderHistories($this->request->get['order_id'], ($page - 1) * 10, 10);
		foreach ($results as $result) {
			$data['histories'][] = array(
		...
		$history_total = $this->model_sale_backorder->getTotalOrderHistories($this->request->get['order_id']);
		$pagination = new Pagination();
		$pagination->total = $history_total;
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('sale/backorder/history', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'] . '&page={page}', 'SSL');
		$data['pagination'] = $pagination->render();
	...
	public function invoice() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$this->load->model('tool/upload');
				$product_data = array();
				$products = $this->model_sale_backorder->getOrderProducts($order_id);
				foreach ($products as $product) {
					$option_data = array();
					$options = $this->model_sale_backorder->getOrderOptions($order_id, $product['order_product_id']);
					foreach ($options as $option) {
						if ($option['type'] != 'file') {
							$value = $option['value'];
						} else {
			...
			$data['commission_total'] = $this->model_marketing_affiliate->getTotalTransactionsByOrderId($this->request->get['order_id']);
				$voucher_data = array();
				$vouchers = $this->model_sale_backorder->getOrderVouchers($order_id);
				foreach ($vouchers as $voucher) {
					$voucher_data[] = array(
						'description' => $voucher['description'],
						'amount'      => $this->currency->format($voucher['amount'], $order_info['currency_code'], $order_info['currency_value'])
				...
				$total_data = array();
				$totals = $this->model_sale_backorder->getOrderTotals($order_id);
				foreach ($totals as $total) {
					$total_data[] = array(
						'title' => $total['title'],
						'text'  => $this->currency->format($total['value'], $order_info['currency_code'], $order_info['currency_value']),
	...
	public function assignsp()
	{
		...
		if (isset($this->request->get['order_id']) && isset($this->request->get['sales_person'])) {
			$this->load->model('sale/backorder');
			$this->model_sale_backorder->assignSalesPersontoOrder($this->request->get['order_id'],$this->request->get['sales_person']);
			$order_status_id = $this->model_sale_backorder->getLatestOrderStatusId($this->request->get['order_id']);
			$sale_person = $this->model_sale_backorder->getSalesPersonById($this->request->get['sales_person']);
			$comment = "Sales person " . $sale_person . " is assigned to this order." ." (" . $this->request->get['username'] . ")";
			$this->model_sale_backorder->addOrderHistory($this->request->get['order_id'],$order_status_id, $comment);
