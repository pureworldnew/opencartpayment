...
class ControllerSaleOrderq extends Controller {
	...
	public function simpleinvoice()
	{
		...
		$order_data = $this->model_sale_orderq->getOrder($order_id);
		if( $order_data )
		{
			...
			$config_fax = $this->config->get('config_fax');
			$config_email = $this->config->get('config_email');
			$config_website = "https://www.gempacked.com/";
			$store_address = $config_name . " " . $config_address . "\n";
			$store_address .= "Telephone: " . $config_telephone . "\n"; 
			$store_address .= "Fax: " . $config_fax . "\n"; 
			$store_address .= "E-mail: " . $config_email . "\n"; 
			$store_address .= "Website: " . $config_website; 

			$order_info = "Date Added: " . date("m/d/Y", strtotime($order_data['date_added'])) . "\n";
			$order_info .= "Order ID: " . $order_id . "\n";
			$order_info .= "Payment Method: " . "\n";
			$order_info .= $order_data['payment_method'] . "\n";
			$order_info .= "Shipping Method: " . "\n";
			$order_info .= $order_data['shipping_method'] . "\n";

			$payment_address = ucfirst($order_data['payment_firstname']) . " " . ucfirst($order_data['payment_lastname']) . "\n";

			$payment_address .= $order_data['payment_address_1'];
			
			if( $order_data['payment_address_2'] != '' )
			{
				$payment_address .= " " . $order_data['payment_address_2'];
			}
			
			$payment_address .= "\n";
			$payment_address .=  $order_data['payment_city'] . " " . $order_data['payment_zone_code'] . " " . $order_data['payment_country'] . "\n" ;
			$payment_address .=  $order_data['email'] . " " . $order_data['telephone'];

			$shipping_address = ucfirst($order_data['shipping_firstname']) . " " . ucfirst($order_data['shipping_lastname']) . "\n";

			$shipping_address .= $order_data['shipping_address_1'];
			
			if( $order_data['shipping_address_2'] != '' )
			{
				$shipping_address .= " " . $order_data['shipping_address_2'];
			}
			
			$shipping_address .= "\n" . $order_data['shipping_city'] . " " . $order_data['shipping_zone_code'] . " " . $order_data['shipping_country'];
			$shipping_address .= "\n" . $order_data['email'] . " " . $order_data['telephone'];

			$order_products = $this->model_sale_orderq->getSimpleInvoiceOrderProduct($order_id);

			$order_totals = $this->model_sale_orderq->getOrderTotals($order_id);
			 
			$writer = new XLSXWriter();

			$writer->setTitle('Invoice');
			$writer->setSubject('Invoice');
			$writer->setAuthor('Gempacked');
			$writer->setCompany('Gempacked');

			$writer->setDescription('Gempacked Invocie');

			$sheet1 = 'Invoice No ' . $order_id;

			// Invoice #
			$format = array('font'=>'Arial','font-size'=>12,'font-style'=>'bold', 'height'=>20);
			$invoice_number = "Invoice # ". $order_id;
			$writer->writeSheetRow($sheet1, $row=array($invoice_number), $format);
			$writer->markMergedCell($sheet1, $start_row=0, $start_col=0, $end_row=0, $end_col=7);

			// Store Info
			$format = array('font'=>'Arial','font-size'=>9,'font-style'=>'bold', 'height'=>85, 'wrap_text'=>true);
			$writer->writeSheetRow($sheet1, $row=array($store_address,"","","",$order_info), $format);

			$writer->markMergedCell($sheet1, $start_row=1, $start_col=0, $end_row=1, $end_col=3);
			$writer->markMergedCell($sheet1, $start_row=1, $start_col=4, $end_row=1, $end_col=7);

			// Blank Row
			$format = array('font'=>'Arial','font-size'=>9,'font-style'=>'bold');
			$writer->writeSheetRow($sheet1, $row=array("","","","",""), $format);
			...
			$format = array('font'=>'Arial','font-size'=>9,'font-style'=>'bold');
			$writer->writeSheetRow($sheet1, $row=array("Payment Address","","","", "Shipping Address"), $format);

			$writer->markMergedCell($sheet1, $start_row=3, $start_col=0, $end_row=3, $end_col=3);
			$writer->markMergedCell($sheet1, $start_row=3, $start_col=4, $end_row=3, $end_col=7);

			$format = array('font'=>'Arial','font-size'=>9,'height'=>50,'wrap_text'=>true);
			$writer->writeSheetRow($sheet1, $row=array($payment_address,"","","", $shipping_address), $format); 

			$writer->markMergedCell($sheet1, $start_row=4, $start_col=0, $end_row=4, $end_col=3);
			$writer->markMergedCell($sheet1, $start_row=4, $start_col=4, $end_row=4, $end_col=7);
			...
			$writer->markMergedCell($sheet1, $start_row=5, $start_col=0, $end_row=5, $end_col=7);
			...
			$writer->writeSheetRow($sheet1, $row=array("Product","","","","Model", "Quantity","Unit Price","Total"), $format);
			$writer->markMergedCell($sheet1, $start_row=6, $start_col=0, $end_row=6, $end_col=3);

			// Items Data

			$format = array('font'=>'Arial','font-size'=>9);
			$i = 7; // start with 7th row
			foreach( $order_products as $order_product )
			{
				$quantity = $order_product['quantity_supplied'] > 0 ? $order_product['quantity_supplied'] : 0;
				$quantity_shipped = $order_product['quantity_supplied'] > 0 ? 1 : 0;
				$product_name = $order_product['parent_category_name'] . " - Pack of " . $quantity . " " . $order_product['unit_name'];
				$product_price = $this->currency->format($order_product['price'], $this->config->get('config_currency'));
				$product_total = $this->currency->format($order_product['total'], $this->config->get('config_currency'));
				$product_model = $order_product['model'];

				$writer->writeSheetRow($sheet1, $row=array($product_name,"","","",$product_model, $quantity_shipped,$product_total,$product_total), $format);
				$writer->markMergedCell($sheet1, $start_row=$i, $start_col=0, $end_row=$i, $end_col=3);
				$i++;
			}
			...
			$writer->markMergedCell($sheet1, $start_row=$i, $start_col=0, $end_row=$i, $end_col=7);
			$i++; 
			...
			{
				$value = $this->currency->format($order_total['value'], $this->config->get('config_currency'));
				$writer->writeSheetRow($sheet1, $row=array($order_total['title'],"","", "","","","",$value), $format);
				$writer->markMergedCell($sheet1, $start_row=$i, $start_col=0, $end_row=$i, $end_col=6);
				$i++;
			}
			
			$file_name = DIR_IMAGE . "/invoice/invoice-" . $order_id . ".xlsx";
			...
			if( !file_exists($file_name) ) 
			{
				...
				$json['success'] = "invoice-" . $order_id . ".xlsx";
			}
		...
		*/
		...
		if (!empty($order_info)) { 
			...
			foreach ($products as $product) {
				if($product['order_item_sort_order'] > 0){
					$order_item_order = $product['order_item_sort_order'];
				}else{
					$order_item_order = $order_item_sort_order;
				}
				$data['order_products'][] = array(
					...
					'price'      => $product['price'],
					'total'      => $product['total'],
					'order_item_sort_order'	=> $order_item_order,
	...
	public function create_backorder() {
		...
		{
			$this->load->model("sale/orderq");
			$this->model_sale_orderq->createBackOrder( array(), $product_id, $customer_id, $order_id); 
		}
		$this->response->addHeader('Content-Type: application/json');
		$this->response->setOutput(json_encode($json));
	}
	...
	public function create_incoming_order() {
		...
		if($product_id > 0)
		{
			...
			$this->model_sale_orderq->addFirstProductToIncomingOrder( $new_incoming_order_id, $product_id, $order_id ); 
		}
		$this->response->addHeader('Content-Type: application/json');
		$this->response->setOutput(json_encode($json));
	}
