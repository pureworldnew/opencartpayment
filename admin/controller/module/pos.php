...
class ControllerModulePos extends Controller {
	...
	public function index() {
		...
		$data['text_print_type_318receipt'] = $this->language->get('text_print_type_318receipt');
	...
	public function autocomplete_product() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			if (!empty($this->request->get['filter_scopes'])) {
				...
				$filter_scopes = array('name','model','upc','sku');
			}
			...
			if (isset($this->request->post['quick_sale'])) {
				$data['quick_sale'] = $this->request->get['quick_sale'];
			}
			if (isset($this->request->post['customer_group_id'])) {
...
//		}
	...
	public function export_report_csv() {
		...
		$data = array(); //$this->config->get('POS_' . $type . '_report_items');
		if (empty($data)) {
			if ($type == 'sales') {
				// set default fields
				$data = array(
					'(order) is_pos' => array('title' => 'Store Name', 'order' => 0, 'type' => 'int(1)'),
					'(order) order_id' => array('title' => 'Order ID', 'order' => 1, 'type' => 'int(11)'),
					'(order_product) product_id' => array('title' => 'Produt ID', 'order' => 2, 'type' => 'int(11)'),
					'(order_product) model' => array('title' => 'Model Number', 'order' => 3, 'type' => 'varchar(64)'),
					'(order_product) sku' => array('title' => 'SKU', 'order' => 4, 'type' => 'varchar(64)'),
					'(order_product) quantity_supplied' => array('title' => 'Quantity Shipped', 'order' => 5, 'type' => 'decimal(55,4)'),
					'(order_product) unit_price' => array('title' => 'Unit Price', 'order' => 6, 'type' => 'int(11)'),
					'(order_product) total' => array('title' => 'Product Total', 'order' => 7, 'type' => 'decimal(15,4)'),
					'(order) total' => array('title' => 'Order Total', 'order' => 8, 'type' => 'decimal(15,4)'),
					'(order) email' => array('title' => 'Customer email', 'order' => 9, 'type' => 'varchar(96)'),
					'(order) name' => array('title' => 'Customer Name', 'order' => 10, 'type' => 'int(11)'),
					'(order_product) groupindicator' => array('title' => 'Group Indicator', 'order' => 11, 'type' => 'int(11)'),
					'(order_product) groupindicator_id' => array('title' => 'Group Indicator ID', 'order' => 12, 'type' => 'int(11)'),
					'(order) order_status_id' => array('title' => 'Order Status', 'order' => 13, 'type' => 'int(11)'),
					'(order_product) product_status' => array('title' => 'Product Status', 'order' => 14, 'type' => 'int(11)'),
					'(order_product) pos_status' => array('title' => 'Pos status', 'order' => 15, 'type' => 'int(11)'),
					'(order) date_added' => array('title' => 'Date Created', 'order' => 16, 'type' => 'datetime'),
					'(order) date_modified' => array('title' => 'Date Modified', 'order' => 17, 'type' => 'datetime'));
			} else {
				$data = array(
					'(product) product_id' => array('title' => 'Product ID', 'order' => 0, 'type' => 'int(11)'),
					'(product) model' => array('title' => 'Model', 'order' => 1, 'type' => 'varchar(64)'),
					'(product) sku' => array('title' => 'SKU', 'order' => 2, 'type' => 'varchar(64)'),
					'(product_description) name' => array('title' => 'Product Name', 'order' => 3, 'type' => 'varchar(255)'),
					'(product) mpn' => array('title' => 'MPN', 'order' => 4, 'type' => 'varchar(64)'),
					'(product) upc' => array('title' => 'UPC', 'order' => 5, 'type' => 'varchar(255)'),
					'(product) price' => array('title' => 'Price', 'order' => 6, 'type' => 'decimal(15,4)'),
					'(product) unique_option_price' => array('title' => 'Unique Option Price', 'order' => 7, 'type' => 'decimal(30,20)'),
					'(product) labour_cost' => array('title' => 'Labour Cost', 'order' => 8, 'type' => 'decimal(30,20)'),
					'(product) unique_price_discount' => array('title' => 'Unique Price Discount', 'order' => 9, 'type' => 'decimal(20,10)'),
					'(product) weight' => array('title' => 'Weight', 'order' => 10, 'type' => 'decimal(15,8)'),
					'(product) location' => array('title' => 'Location', 'order' => 11, 'type' => 'varchar(128)'),
					'(product) quantity' => array('title' => 'Total Instock', 'order' => 12, 'type' => 'int(4)'),
					'(product) location_by_quantity' => array('title' => 'Location By Quantity', 'order' => 13, 'type' => 'varchar(128)'),
					'(product) status' => array('title' => 'Status', 'order' => 14, 'type' => 'tinyint(1)'),
					'(product) pos_status' => array('title' => 'POS Status', 'order' => 15, 'type' => 'tinyint(1)'),
					'(manufacturer) name' => array('title' => 'Brand Name', 'order' => 16, 'type' => 'varchar(64)'));
			}
		}
