...
class ControllerModulesupermenu extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('module/supermenu', 'token=' . $this->session->data['token'] . $redirect, 'SSL'));
		}
		...
		if ($data['store']) {
		  $data['action'] = $this->url->link('module/supermenu', 'token=' . $this->session->data['token'] . '&store=' . $data['store'], 'SSL');
		} else {
		  $data['action'] = $this->url->link('module/supermenu', 'token=' . $this->session->data['token'], 'SSL');
		}

		$data['module_url'] = $this->url->link('module/supermenu', 'token=' . $this->session->data['token'], 'SSL');
