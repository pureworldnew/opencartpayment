...
class ControllerToolKaOrderImport extends KaController { 
	...
	protected function onLoad() {
		...
		$this->data['heading_title']    = "CSV Incoming Order Import";
		...
		$this->data['breadcrumbs'][] = array(
			'text'      => "CSV Incoming Order Import",
			'href'      => $this->url->link('tool/ka_order_import', 'token=' . $this->session->data['token'], 'SSL'),
		);	
	...
	protected function prepareOutput() {
		$this->document->setTitle("CSV Incoming Order Import");
		
		if (empty($this->template)) {
			$this->template = 'tool/ka_order_import.tpl';
		}
	...
	public function index() { // step1
		...
		$this->data['action']    = $this->url->link('tool/ka_order_import/import', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function import()
	{
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if ($this->request->post['mode'] == 'load_profile') {
				...
				return $this->redirect($this->url->link('tool/ka_order_import', 'token=' . $this->session->data['token'], 'SSL'));
			...
			if ($this->params['location'] == 'server') {
				...
				if (empty($this->params['file'])) {
					...
					$validate = $this->model_tool_ka_order_import->validateFile($this->params['file'],$this->params['delimiter']);
					if ( $validate != "ok" )
					{
						$msg = $msg . $validate;
					}
			...
			// Import Begins
			$this->model_tool_ka_order_import->processImport($params['file'], $params['delimiter']);

			$this->session->data['success'] = "Incoming Orders Successfully Imported."; 
