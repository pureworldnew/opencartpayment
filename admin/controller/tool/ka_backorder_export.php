...
class ControllerToolKaBackorderExport extends KaController { 
	...
	protected function onLoad() {
		...
		$this->data['heading_title']    = "CSV BackOrder Export";
		...
		$this->data['breadcrumbs'][] = array(
			'text'      => "CSV BackOrder Export",
			'href'      => $this->url->link('tool/ka_backorder_export', 'token=' . $this->session->data['token'], 'SSL'),
		);	
	...
	protected function prepareOutput() {
		$this->document->setTitle('CSV BackOrder Export');
	...
	public function index() { // step1
		...
		if (!$this->model_tool_ka_product_export->isDBPrepared()) {
			...
			$this->template = 'tool/ka_backorder_export.tpl';
			$this->prepareOutput();
			return;
		}
		...
		$this->data['action']    = $this->url->link('tool/ka_backorder_export/export', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function export()
	{
		if ($this->request->post['products_from_manufacturers'] == 'selected') {
			if (empty($this->request->post['manufacturer_ids'])) {
        		...
        		$this->response->redirect($this->url->link('tool/ka_backorder_export', 'token=' . $this->session->data['token'], 'SSL'));
			}
		}
		$this->load->model('tool/ka_order_export');
		$this->model_tool_ka_order_export->exportBackOrders($this->request->post);
	}
