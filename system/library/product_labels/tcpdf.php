...
class TCPDF {
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		/*if we are here it means image file path exists but it points to a damaged, not valid image
		  so this will cause pdf label printing to fail and stop the process. But some products not all
		  will have that problem. So we simply ignore those images and continue with printing.
		*/
		...
		if ($imsize === FALSE) {
			if (($w > 0) AND ($h > 0)) {
				...
				//use some default not found image rather than terminating whole process. 
				$file = HTTP_SERVER . "catalog/view/theme/default/image/no_product.jpg";
				$file = str_replace('/admin','',$file); //replace admin path with catalog path
				$imsize = @getimagesize($file); //set image to that file
