...
class ProductLabels extends TCPDF {
			...
			foreach($DATA['type'] as $row => $type) {
				...
				switch($type) {
					...
					case "rect":
						...
						eval("\$rectw = ".str_replace("width",$w,$DATA['w'][$row]).";");
						eval("\$recth = ".str_replace("height",$h,$DATA['h'][$row]).";");
					...
					case "img":
						...
						if (strpos($DATA['img'][$row],'image}') !== false) {
							$img_name = $this->parseText($DATA['img'][$row],$DATA);
							if($img_name == "") $img_name = "placeholder.png";
							$img = DIR_IMAGE.$img_name;
						} else {
							$DATA['img'][$row] = $this->parseText($DATA['img'][$row],$DATA);
							$img = str_replace("//","/", DIR_IMAGE.parse_url($DATA['img'][$row], PHP_URL_PATH));
						}
						
						if (strpos($DATA['img'][$row],'qrcode') !== false) {
							//echo $DATA['product_info']['qrformat'];
							$DATA['product_info']['qrformat'] = $this->parseQrFormat($DATA['product_info']['qrformat'],$DATA);
							//echo $DATA['product_info']['qrformat'];
							//die();
							$style = array(
								'border' => 0,
							...
							$this->write2DBarcode($DATA['product_info']['qrformat'], 'QRCODE,Q', $DATA['x'][$row], $DATA['y'][$row], $DATA['w'][$row], $DATA['h'][$row], $style, 'N');
						}
	...
	function parseQrFormat($format,$data = array()) {
		$qrcode_string = "";
		$fields = "";
		if ($format != null) {
			$fields = $format;
		}else{
			$fields = $data['product_info']['qrcode'];
		}
	...
	function parseText($text, $product) {
		...
		foreach(array('product_id', 'name', 'model', 'sku', 'upc', 'ean', 'jan', 'isbn', 'mpn', 'location', 'quantity', 'image', 'manufacturer', 'manufacturer_image') as $key)
		...
		$replace['{pricetax}'] = isset($product['product_info']['price'])?number_format($product['product_info']['price_tax'], 2, '.', ','):"";
