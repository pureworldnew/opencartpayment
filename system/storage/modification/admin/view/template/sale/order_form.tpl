              ...
              <div class="form-group">
                ...
                <div class="col-sm-10">
                  <select name="store" id="input-store" class="form-control">
                    ...
                    <option value="<?php echo str_replace('http','https',$store['href']); ?>" selected="selected"><?php echo $store['name']; ?></option>
                    <?php } else { ?>
                    <option value="<?php echo str_replace('http','https',$store['href']); ?>"><?php echo $store['name']; ?></option>
                    <?php } ?>
                    <?php } ?>
                  </select>
                </div>
...
<?php if ($prm_access_permission && $laccess) { ?>
                      <td class="text-right"><?php echo $column_cost; ?></td>
<?php } ?>
                  ...
                  <fieldset>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-10">
                        ...
                        <input type="hidden" name ="groupbyvalue" value=""> 
                        <input type="hidden" name ="group_indicator_id" value=""> 
                      </div>
                    </div>
                    <div class="form-group">
...
<?php } ?>
			...
			<legend>Adding Custom Total, Shipping, Payment</legend>
					<div class="col-sm-4">
                    <div class="table-responsive">
                      <table class="table table-bordered">
                        <thead>
                          <tr>
                            <td class="text-left">Custom Fee Text</td>
                	...
                	<div class="col-sm-8">
	                    ...
	                    <div class="table-responsive">
	                      <table class="table table-bordered">
	                        <thead>
	                          <tr>
	                            <td class="text-left">Custom Shipping</td>
	                            <td class="text-left">Taxes</td>
	                            <td class="text-left">Amount</td>
	                          </tr>
	                        </thead>
	                        <tbody id="customtotal_shipping">
	                          <tr>
	                              <td><input type="text" class="form-control" name="customtotal_shipping[name]" value="<?php echo $shipping_method_name; ?>" ></td>
	                              <td>
	                    ...
	                    <div class="table-responsive">
	                      <table class="table table-bordered">
	                        <thead>
	                          <tr>
	                            <td class="text-left">Custom Payment</td>
	                            <td class="text-left">Amount</td>
	                          </tr>
	                        </thead>
	                        <tbody id="customtotal_payment">
	                          <tr>
	                           	<td><input type="text" class="form-control" name="customtotal_payment[name]" value="<?php echo $payment_method_name; ?>" ></td>
...
$(document).delegate('#tab-cart', 'click', function() {
		...
		success: function(json) {
			...
			if (json['products'].length) {
				for (i = 0; i < json['products'].length; i++) {
					...
					/*<div class="input-group btn-block" style="max-width: 200px;"><input type="text" name="product[' + i + '][quantity]" value="' + product['quantity'] + '" class="form-control" /><span class="input-group-btn"><button type="button" data-toggle="tooltip" title="<?php echo $button_refresh; ?>" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary"><i class="fa fa-refresh"></i></button></span></div></td>';*/
					if(product['unit']['unit_conversion_values']) {
						html += '  <input type="hidden" name="product[' + i + '][convert_quantity]" value="' + quantity + '" />';
						html += '  <input type="hidden" name="product[' + i + '][quantity]" value="' + quantity + '" />';
						...
						html += product['quantity'] + ' ' + product['DefaultUnitName']['unit_plural'] +'</td>';
                    }
					html += '  <td class="right">' + '<input type="text" name="order_product[' + i + '][quantity_supplied]" value="' + product['quantity_supplied'] + '" /></td>';
					// Line # 1597
			...
			if (json['products'].length) {
				for (i = 0; i < json['products'].length; i++) {
					...
					if(product['unit'] && product['unit'] != ''){
                        html += '  - <small>' + product['unit']['unit_name'] + ': ' + product['unit']['unit_value_name'] + '</small><br />';
                    }
					...
					*/ //+ product['quantity'] + '</td>';
					html += '  <td class="text-right">';
					html += product['quantity_supplied'] + '</td>';
					html += '  <td class="text-right">';
					if(product['unit']) {
                               	html += price + '</td>';
                   		}else{ 
								html += product['price'] + '</td>';
                   		} 
					 //+ product['price'] + '</td>';
...
$('#tab-product input[name=\'product\']').autocomplete({
	...
	'select': function(item) {
		...
		$('#tab-product input[name=\'groupbyvalue\']').val(item['groupdata']['data']['requested_product_data']['groupbyvalue']);
		$('#tab-product input[name=\'group_indicator_id\']').val(item['groupdata']['data']['group_indicator_id']);
		if (item['unit'] != '' || item['unit'] != 0) {
			
                    template = '';
                    for (i = 0; i < item['unit'].length; i++) {
                        
                        html1 = '<fieldset>';
						html1 += '  <legend>Choose Unit</legend>';
                        unit = item['unit'][i];
                        html1 += '<div class="form-group" id="unit-' + unit[i] + '">';
						html1 += '<label class="col-sm-2 control-label" for="input-option' + unit['unit_name'] + '">' + unit['unit_name'] + '</label>';
						html1 += '<div class="col-sm-10">';
               
                        html1 += '<select id ="get-unit-data" name="unit['+i+']" class="form-control">';
						
                        for (j = 0; j < unit['unit_value'].length; j++) {
                                unit_value = unit['unit_value'][j];
                                if(unit_value['unit_conversion_values'] == item['groupdata']['DefaultUnitdata']['unit_conversion_product_id']){
									selected = ' selected ';
								}else{
										selected = '';
								}
                                html1 += '<option '+selected+' data-value ="' + unit_value['unit_conversion_values'] + '" value="' + unit_value['unit_value_name'] + '">' + unit_value['unit_value_name'];
             ...
             $('#unit').html('<div class="left"></div><div class="left">' + template + '</div>');
              addUnit();
		...
		if (item['option'] != '') {
			...
			for (i = 0; i < item['option'].length; i++) {
				...
				if (option['type'] == 'select') {
					...
					html += '    <select onchange="selectproductoptions();" name="option[' + option['product_option_id'] + ']" id="input-option' + option['product_option_id'] + '" class="form-control">';
					...
					for (j = 0; j < option['product_option_value'].length; j++) {
						...
						html += '<option ' + selected + ' value="' + option_value['product_option_value_id'] + '">' + option_value['name'];
						html += '</option>';
					}
...
function addUnit(){
        var coversion_id = $("#unit:visible").find('option:selected').attr('data-value');
...
function selectproductoptions(){
	...
	var that=this;
      ...
      /*  if ($('#unit').children().length > 0) {
		...
		//var tokenr = <?php echo $token;?>;
        var groupbyvalue = $('input[name=groupbyvalue]').val();
		var group_indicator_id = parseInt($('input[name=group_indicator_id]').val());
            ...
            data: {
              ...
              //  "unit_conversion": unit_conversion,
               // 'unit_conversion_text': unit_conversion_text,
                'group_indicator': group_indicator_id,
                'selChoice':selOpt,
                'groupbyvalue':groupbyvalue
            },
			success: function(resp){
				$('#tab-product input[name=\'product_id\']').val(resp['product_id']);
				html = '';
				...
				selectedval = $('#get-unit-data').val();
					...
					if (resp['option'] != '') {
						...
						for (i = 0; i < resp['option'].length; i++) {
				option = resp['option'][i];
...
if (!json['error']) {
          var adminmonitor_order_id = json['order_id'] ? json['order_id'] : $('input[name=\'order_id\']').val();
          var adminmonitor_event_type = json['order_id'] ? 'add' : 'edit';
