	...
	public function addProduct($data) {
		...
		if (!empty($data['product_article'])) {
			foreach ($data['product_article'] as $product_article) {
                    ...
                    if (isset($product_article['article_id']) && $product_article['name']!='') {
                
					
                   $this->db->query("INSERT INTO " . DB_PREFIX . "product_article SET product_id = '" . (int)$product_id . "', article_id = '" . (int)$product_article['article_id'] . "', name = '" . $this->db->escape($product_article['name']) . "',display_text = '" . $this->db->escape($product_article['display_text']) . "', sort_order= '".(int)$product_article['sort_order']."',embed_in_description='".(int)$product_article['embed_in_description']."'");
...
if ($product_option_value['cost'] > 0 && $product_option_value['cost_amount'] == 0) {			
	...
	$cost_amount = (float)$product_option_value['cost_amount'];
							...
							$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . (int)$product_option_value['option_value_id'] . "', quantity = '" . (int)$product_option_value['quantity'] . "', subtract = '" . (int)$product_option_value['subtract'] . "', price = '" . (float)$product_option_value['price'] . "', price_prefix = '" . $this->db->escape($product_option_value['price_prefix']) . "', cost = '" . (float)$product_option_value['cost'] . "', cost_amount = '" . $cost_amount . "', costing_method = '" . (int)$product_option_value['costing_method'] . "', cost_prefix = '" . $this->db->escape($product_option_value['cost_prefix']) . "', sku = '" . $this->db->escape($product_option_value['sku']) . "', points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->db->escape($product_option_value['points_prefix']) . "', weight = '" . (float)$product_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($product_option_value['weight_prefix']) . "'");
