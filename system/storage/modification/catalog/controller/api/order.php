...
class ControllerApiOrder extends Controller {
	public function add() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if(isset($this->session->data['custom_payment_method'])) {
				$this->session->data['payment_method']  = $this->session->data['custom_payment_method'];
			}
			...
			if(isset($this->session->data['custom_shipping_method'])) {
				$this->session->data['shipping_method']  = $this->session->data['custom_shipping_method'];
			}
	...
	public function history() { 
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($order_info) {
			$order_status_id = $this->model_checkout_order->getLatestOrderStatusId($order_id);
			if($order_status_id != $this->request->post['order_status_id']) 
			{
				if( $order_status_id == 1 && !empty($this->request->post['username']) )
				{
					$sale_person = $this->model_checkout_order->getSalesPersonByName($this->request->post['username']);
					$this->model_checkout_order->assignSalesPersontoOrder($order_id,$sale_person);
					$this->request->post['comment'] .= "Sales person " . $this->request->post['username'] . " is assigned to this order.";
				}
				$order_status_from = $this->model_checkout_order->getOrderStatus($order_status_id);
				$order_status_to = $this->model_checkout_order->getOrderStatus($this->request->post['order_status_id']);
				$this->request->post['comment'] .= "Order status changed from " . $order_status_from . " to " . $order_status_to . ".";
			}
			if(!empty($this->request->post['sales_person']))
				{
					$this->model_checkout_order->assignSalesPersontoOrder($order_id,$this->request->post['sales_person']);
					$sale_person = $this->model_checkout_order->getSalesPersonById($this->request->post['sales_person']);
					$this->request->post['comment'] .= "Sales person " . $sale_person . " is assigned to this order.";
				}
			
			$this->request->post['comment'] .= " (" . $this->request->post['username'] . ")";
	...
	public function changestore() {
		...
		$this->model_checkout_order->changeStore($order_id, $this->request->post['store_type']);
		$this->load->language('api/payment');

		$json = array();

		$json['success'] = "Store Changed Successfully.";
		$json['store_type'] = $this->request->post['store_type'];
