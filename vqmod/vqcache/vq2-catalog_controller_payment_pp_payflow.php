...
// 			'value' => 'S'
	...
	public function send() {
		...
		$request .= '&NAME=' . $this->request->post['cc_owner'];
...
//Q: Paypal Admin Tools
		...
		if ($response_info['RESULT'] == '0') {
			...
			if (isset($response_info['AUTHCODE'])) {
				$message .= 'Authorization Code: ' . $response_info['AUTHCODE'] . "\n";
			}

			if (isset($response_info['RESPMSG'])) {
				$message .= 'Response MSG: ' . $response_info['RESPMSG'] . "\n";
			}
			...
			if (isset($response_info['PPREF'])) {
				$message .= 'Transactional ID: ' . $response_info['PPREF'] . "\n";
			}
