	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_company_id'])) {
				$url .= '&filter_company_id=' . urlencode(html_entity_decode($this->request->get['filter_company_id'], ENT_QUOTES, 'UTF-8'));
			}
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_email'])) {
			...
			foreach ($results as $result) {
				...
				$json[] = array(
'oname'			=> strip_tags(html_entity_decode($result['oname'], ENT_QUOTES, 'UTF-8')),
'oname'			=> strip_tags(html_entity_decode($result['oname'], ENT_QUOTES, 'UTF-8')),
					'customer_id'       => $result['customer_id'],
