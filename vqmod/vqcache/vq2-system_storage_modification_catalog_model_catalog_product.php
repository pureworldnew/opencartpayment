...
class ModelCatalogProduct extends Model {
	...
	public function getProductArticles($product_id) {
		$product_articles = array();
			 ...
			 $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_article WHERE product_id = '" . (int) $product_id . "' order by sort_order");
		...
		if($result)
		{
			foreach($result as $row)
			{
			 ...
			 if($row['article_id']&&$row['embed_in_description'])
				    $product_articles[] = $this->getInformationDetails($row['article_id']);
                else
				    $product_articles[] = $row;
    ...
    public function getWeightClasses($unit1, $unit2) {
		...
		if ($query->num_rows) {
			...
			return false;
                    ...
                    public function getQuickProduct($product_name){
                  		$result = $this->db->query("SELECT pd.name , p.price , p.product_id , p.model, p.minimum , p.tax_class_id FROM ".DB_PREFIX."product p LEFT JOIN ".DB_PREFIX."product_description pd ON (p.product_id = pd.product_id) WHERE (p.model LIKE '%".$product_name."%' || p.sku LIKE '%".$product_name."%' || pd.name LIKE '%".$product_name."%' || pd.meta_title LIKE '%".$product_name."%' || pd.meta_keyword LIKE '%".$product_name."%') AND p.quantity > 0 AND p.status = 1 LIMIT 10")->rows;

                  		foreach($result as $key => $value){
                  			$result[$key]['hasoption'] = $this->hasProductOption($value['product_id']);
                  		}
                  		return $result;
                  	}
                		public function hasProductOption($product_id){
                  		return $this->db->query("SELECT * FROM ".DB_PREFIX."product_option WHERE product_id = '".(int)$product_id."'")->num_rows;

                  	}
                		public function getProductQuantity($product_id){
                			return $this->db->query("SELECT quantity FROM ".DB_PREFIX."product WHERE product_id = '".(int)$product_id."'")->row['quantity'];
                		}

                    public function optionValidate($product_id, $option = array(), $recurring_id = 0) {
	...
	public function getLatestSubCatProducts($subcats) {
		...
		return $product_data;
				...
				public function virtualStatus() {
					$module = $this->config->get('latest_virtualcat');
				...
				public function virtualId() {
					...
					return "XYZ"; //$module['category_id'];
				}
				
				public function virtualSort() {
					$module = $this->config->get('latest_virtualcat');
					
					return $module['sort_latest'];
				}
				
				public function virtualSortdefault() {
					$module = $this->config->get('latest_virtualcat');
					
					return $module['sort_default'];
				}
				
				public function virtualSortOption() {
					$module = $this->config->get('latest_virtualcat');
					
					if(!isset($module['sort_option'])) {
						return false;
					} else {				
						return $module['sort_option'];
					}
				}
				
				public function getLatestProductscountbydate() {
					
					// Date limit
					$module = $this->config->get('latest_virtualcat');

					$sqlgetlatest = "SELECT p.product_id, p.date_added FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_added >= CURDATE() - INTERVAL '" . (int)($module['datelimit']-1) . "' DAY AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.date_added DESC";

					$querys = $this->db->query($sqlgetlatest);
					return count($querys->rows);
				}
				
				public function getLatestProductsbycat($data = array()) {
					if ($this->customer->isLogged()) {
