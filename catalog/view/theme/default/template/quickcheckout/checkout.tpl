...
<div class="container">
  ...
  <div class="row"><?php echo $column_left; ?>
    ...
    <?php $class = 'col-sm-12'; ?>
    <?php } ?>
    <div id="content" class="<?php echo $class; ?>" style="min-height:60px !important">
    		<?php echo $content_top; ?>
              ...
              <link rel="stylesheet" media="screen and (min-width: 901px) and (max-width: 99999px)" href="<?php echo $stylesheet; ?>" />
              <link rel="stylesheet" media="screen and (min-width: 1px) and (max-width: 450px)" href="<?php echo $mobile_stylesheet; ?>" />
              <link rel="stylesheet" media="screen and (min-width: 451px) and (max-width: 900px)" href="<?php echo $tablet_stylesheet; ?>" />
              <?php } else { ?>
		...
		<div id="quickcheckout-disable">
		  ...
		  <?php } elseif ($quickcheckout_layout == '1' || $quickcheckout_layout == '3') { ?>
			...
			<div class="quickcheckoutleft extra-width">
			 	<div id="cart-voucher-resalenumber"></div>
			</div>
			<?php } ?>
		  <?php } ?>
      ...
      <!-- Close QuickCheckoutConfirm div -->
...
$('#button-resale_id_number').live('click', function() {
...
function loadCart() {
	$.ajax({
		...
		beforeSend: function() {
			...
			moduleLoad($('#cart-voucher-resalenumber'), <?php echo $loading_display; ?>);
		},
		success: function(html) {
			moduleLoaded($('#cart-voucher-resalenumber'), <?php echo $loading_display; ?>);
			
			$('#cart-voucher-resalenumber').html(html);
...
var clearsetinterval = function(){
     if(varCounter < 1) {
          varCounter++;
          /* your code goes here */
          $.unblockUI();  $("#button-payment-method").trigger('click'); 
     } else {
          clearInterval(intervalId);
     }
};

$("#button-verification-method").click(function(){

		$.blockUI({
					message: '<h1 style="color:#ffffff;"> Please wait order is verifying.</h1>',
					css: {
						border: 'none',
						padding: '15px',
						backgroundColor: '#000000',
   ...
   loadCart();
intervalId = setInterval(clearsetinterval, 3000);
...
<?php if ($shipping_required) { ?>
	...
	$('#shipping-and-billing-same').on('change', function() {

		if ($('#shipping-and-billing-same:checked').val()) {
			$('#shipping-address').hide();
			var firstname =	$('#payment-address input[name=\'firstname\']').val();
			var lastname =	$('#payment-address input[name=\'lastname\']').val();
			var company =	$('#payment-address input[name=\'company\']').val();
			var address_1 =	$('#payment-address input[name=\'address_1\']').val();
			...
			$('#shipping-address input[name=\'firstname\']').val(firstname);
			$('#shipping-address input[name=\'lastname\']').val(lastname);
			$('#shipping-address input[name=\'company\']').val(company);
			$('#shipping-address input[name=\'address_1\']').val(address_1);
			$('#shipping-address input[name=\'city\']').val(city);
			$('#shipping-address input[name=\'postcode\']').val(postcode);
			$('#shipping-address select[name=\'country_id\']').val(country_id);
			$('#shipping-address select[name=\'zone_id\']').val(zone_id); 
	  } else {
			$('#shipping-address input[name=\'firstname\']').val("");
			$('#shipping-address input[name=\'lastname\']').val("");
			$('#shipping-address input[name=\'company\']').val("");
			$('#shipping-address input[name=\'address_1\']').val("");
