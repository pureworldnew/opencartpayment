...
class ModelAccountOrder extends Model {
	...
	public function getOrders($start = 0, $limit = 20) {
		...
		$query = $this->db->query("SELECT o.order_id, o.firstname, o.lastname, os.name as status, o.date_added, o.total, o.currency_code, o.currency_value FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_status os ON (o.order_status_id = os.order_status_id) WHERE o.customer_id = '" . (int)$this->customer->getId() . "' AND o.order_status_id > '0' AND o.order_type NOT IN (2,3) AND o.store_id = '" . (int)$this->config->get('config_store_id') . "' AND os.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY o.order_id DESC LIMIT " . (int)$start . "," . (int)$limit);
	...
	public function eligibleToCombine( $customer_id, $time_frame, $selected_order_statuses )
	{
		$time_frame = "-" . $time_frame . " day";
		$time_frame_date = date("Y-m-d", strtotime($time_frame));
		$query = $this->db->query("SELECT order_id FROM `" . DB_PREFIX . "order` WHERE customer_id = '" . (int)$customer_id . "' AND order_status_id IN (" . $selected_order_statuses .") AND date_added >= DATE('" . $time_frame_date . "')");
