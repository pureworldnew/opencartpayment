...
class ModelCatalogaSearch extends Model {
	...
	public function GetImageForOption($opt_id,$option_value_id, $product_id){
		
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option_value pov WHERE 
								
				pov.product_id = '" . (int)$product_id . "' 
				AND pov.option_id = '" . (int)$opt_id ."' 
				...
				if ($query->num_rows) {
					...
					return $this->getoptionImagesize($query->row['opt_image'],$this->config->get('config_image_product_width'), $this->config->get('config_image_product_height'));
	...
	public function getoptionImagesize($image, $s1, $s2) { 
	
	 	$this->load->model('tool/image');
	
		if (!empty($image) && file_exists(DIR_IMAGE . $image)) {
					
			$image = $this->model_tool_image->resize($image, $s1, $s2);
	 	}else { 
			$image= ''; }
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_category_id'])) {
			if (!empty($data['filter_sub_category'])) {
				...
				$cat_array = $this->getCategoriesByParentId($data['filter_category_id']);
        		$cat_id = implode(',',$cat_array);
				$sql .= " AND p2c.category_id IN ($cat_id)";		
			}	
