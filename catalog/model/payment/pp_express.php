...
class ModelPaymentPPExpress extends Model {
	...
	public function call($data) {
		//print_r($this->session->data['additional_cost_for_wire']); 
		//die($this->session->data['additional_cost_for_wire']);
	...
	public function paymentRequestInfo() {
		...
		$data['PAYMENTREQUEST_0_ADDITIONALCOST'] = '';
		...
		if (!empty($this->session->data['additional_cost_for_wire'])) {
			$additional_amt = $this->session->data['additional_cost_for_wire'];
		}
		...
		foreach ($this->cart->getProducts() as $item) {
			...
			//	$additional_amt += $item['additional'];
				//$item_price = $this->currency->format($item['price'] + ($item['additional']/$item['quantity']), false, false, false);	
			//} 
			//}
			//$item_price += (($item_price*10)/100);

			if ( strpos( $item['quantity'], "." ) !== false ) {
				$item_name = $item['name'];
				$item_name .= " - " . $item['quantity'];
				if(!empty($item['unit']['unit_value_name']))
				{
					$item_name .= " " . $item['unit']['unit_value_name'];
				}
				$data['L_PAYMENTREQUEST_0_NAME' . $i] = $item_name;
				$data['L_PAYMENTREQUEST_0_NUMBER' . $i] = $item['model'];
				$data['L_PAYMENTREQUEST_0_AMT' . $i] = $this->currency->format($item['price'] * $item['quantity'], false, false, false);
		...
		if ($additional_amt > 0){
				...
				$data['L_PAYMENTREQUEST_0_DESC' . $i] = '';
				$data['L_PAYMENTREQUEST_0_NAME' . $i] = 'Additional cost of wire';
				$data['L_PAYMENTREQUEST_0_NUMBER' . $i] = 'ADDITIONALCOST';
				$data['L_PAYMENTREQUEST_0_QTY' . $i] = 1;
				$data['L_PAYMENTREQUEST_0_AMT' . $i] = $this->currency->format($additional_amt, false, false, false);
				$i++;
			}
		...
		$data['PAYMENTREQUEST_0_ITEMAMT'] = number_format($item_total+$additional_amt, 2, '.', '');
		$data['PAYMENTREQUEST_0_AMT'] = number_format($item_total+$additional_amt, 2, '.', '');
