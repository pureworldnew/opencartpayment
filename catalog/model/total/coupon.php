...
class ModelTotalCoupon extends Model {
	...
	public function confirm($order_info, $order_total) {
		...
		if ($code) {
			...
			if ($coupon_info) {
				if($this->ExistHistory($order_info['order_id']))
				{
					$this->UpdateHistory($order_info, $coupon_info);
				}
				else
				{
	...
	public function ExistHistory($order_id)
	{
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "coupon_history WHERE order_id = '" . (int)$order_id . "'");
		return $query->num_rows;
	}
	
	public function UpdateHistory($order_info, $coupon_info)
	{
		$this->db->query("UPDATE `" . DB_PREFIX . "coupon_history` SET coupon_id = '" . (int)$coupon_info['coupon_id'] . "', order_id = '" . (int)$order_info['order_id'] . "', customer_id = '" . (int)$order_info['customer_id'] . "', amount = '" . (float)$order_total['value'] . "' WHERE coupon_id = '".(int)$coupon_info['coupon_id'])."'";
	}
