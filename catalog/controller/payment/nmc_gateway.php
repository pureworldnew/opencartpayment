...
class ControllerPaymentNMCGateway extends Controller {
	public function index() { 
		...
		$data['is_usa_customer'] = $this->session->data['payment_address']['country_id'] == 223 ? true : false;
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/nmc_gateway.tpl')) {			
			return $this->load->view($this->config->get('config_template') . '/template/payment/nmc_gateway.tpl', $data);
		} else {			
			return $this->load->view('default/template/payment/nmc_gateway.tpl', $data);
		}
	}

	public function send() {

		if ($this->config->get('nmc_gateway_server') == 'live') {
			$url = 'https://webhost.nmconline.com/WebHost2011/WebHost.aspx';			        
		} elseif ($this->config->get('nmc_gateway_server') == 'test') {
			$url = 'https://gateway.nmconline.com/WebHost2011/WebHost.aspx';			        
		}
		...
		$data['x_login'] = $this->config->get('nmc_gateway_login');
		$data['x_tran_key'] = $this->config->get('nmc_gateway_key');
		$data['x_version'] = '3.1';
		...
		$data['x_method'] = 'CC';
		$data['x_type'] = ($this->config->get('nmc_gateway_method') == 'sale') ? 'AUTH_CAPTURE' : 'AUTH_ONLY';
		...
		if (curl_error($curl)) {
			...
			$this->log->write('NMCGATEWAY CURL ERROR: ' . curl_errno($curl) . '::' . curl_error($curl));
		} elseif ($response) {
			$i = 1;
			...
			if ($response_info[1] == '0') {
				...
				//if (!$this->config->get('nmc_gateway_hash') || (strtoupper($response_info[38]) == strtoupper(md5($this->config->get('nmc_gateway_hash') . $this->config->get('nmc_gateway_login') . $response_info[7] . $this->currency->format($order_info['total'], $order_info['currency_code'], 1.00000, false))))) {
					$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('nmc_gateway_order_status_id'), $message, false);
				//} else {
			...
			$this->log->write('NMCGATEWAY CURL ERROR: Empty Gateway Response');
		}
