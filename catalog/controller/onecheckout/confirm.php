...
class ControllerOneCheckoutConfirm extends Controller {
    ...
    public function index() {
        ...
        if (!$json) {  
            ...
            $data['button_confirm'] = $this->language->get('button_confirmorder');
            ...
            foreach ($this->cart->getProducts() as $product) {
                ...
                $data['products'][] = array(
                    ...
                    'tax' => $version_tax,
                    'price' => ($version_int < 1540) ? $this->currency->format($product['price']) : $this->currency->format($this->tax->calculate(($product['price']/$product['unit']['convert_price']), $product['tax_class_id'], $this->config->get('config_tax'))),
                    'total' => ($version_int < 1540) ? $this->currency->format($product['total']) : $this->currency->format($this->tax->calculate(($product['price']/$product['unit']['convert_price']), $product['tax_class_id'], $this->config->get('config_tax')) * ($product['quantity'] * $product['unit']['convert_price'])),
            ...
            //cart module
            $data['cartmodule'] = $this->getChild('onecheckout/cartmodule');
        ...
        if (!$json) {
            ...
            if ($this->cart->hasShipping()) {
                ...
                if (isset($this->session->data['shipping_method']['title'])) {
                    $ship_method = explode('<br />',$this->session->data['shipping_method']['title']);
                    
                    $data['shipping_method'] = $ship_method[0];
                } else {
