...
class ControllerFeedGoogleBase extends Controller {
				...
				foreach ($products as $product) {  
					if (!in_array($product['product_id'], $product_data) && $product['description']) {
						...
						if ( (float)$product['special'] ) {
							...
							$price = $this->getDiscountedPrice(1, $product['product_id']);
							$price = round($price,2);
							$output .= '<g:price>' . $price . '</g:price>';
						}
						...
						if( $price >= $product['minimum_amount'] )
						{
							$output .= '<g:minimum_quantity>1</g:minimum_quantity>';
						} else {
							$minimum_quantity = ceil($product['minimum_amount'] / $price) ;
							$output .= '<g:minimum_quantity>' . $minimum_quantity . '</g:minimum_quantity>';
						}
	...
	public function getDiscountedPrice($discount_qty, $product_id) {
        $this->load->model('catalog/product');
