...
class ControllerProductSearch extends Controller {
	public function index() {
		...
		if (isset($this->request->get['getAllRelated'])) {
			$this->document->setTitle("Related Products");
			$is_related = true;
		...
		if (isset($this->request->get['getAllRelated'])) {
			$url .= '&getAllRelated&product_id=' . $this->request->get['product_id'];
		}
		...
		if (isset($this->request->get['search']) || isset($this->request->get['tag']) || $is_related) {
			$filter_data = array(
				'filter_name'         => $search,
				'filter_tag'          => $tag,
			...
			if ($is_related){
				$results = $this->model_catalog_product->getProductRelated($pid,2,$filter_data);
				$product_total = $this->model_catalog_product->getProductRelatedTotal($pid,$filter_data);
				//echo $product_total;
			}else{
			...
			foreach ($results as $product_id => $result) {
				...
				if( strtolower($result['model']) == trim(strtolower($search)) || strtolower($result['sku']) == trim(strtolower($search)) )
				{
					$re_arrange[$product_id] = $result;
				}

			}
			
			$results = $re_arrange + $results;
			...
			//$pagination->url = 'index.php?route=product/search'. $url . '&page={page}'; //for avoiding rewrites
