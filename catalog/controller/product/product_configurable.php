<?php 
/* 
  #file: catalog/controller/product/product_configurable.php
...
class ControllerProductProductConfigurable extends Controller {
	private $error = array(); 
	
	public function index() {
		$this->language->load('product/product_grouped_mask');
		$this->language->load('product/product_configurable');
...
// S 1/7 //
		$gp_tpl_q = $this->model_catalog_product_configurable->getProductGroupedType($product_id);
		if(!$gp_tpl_q){
		...
		}elseif($gp_tpl_q && $gp_tpl_q['pg_type'] != 'configurable'){
...
// S 3/7 //
			...
			*/
			...
			if((float)$product_info['price']){
				$this->data['price_fixed'] = $this->currency->format($this->tax->calculate($product_info['price'],$product_info['tax_class_id'],$tcg_config_tax));
			}else{
				$this->data['price_fixed'] = false;
			}
			if($tcg_config_tax){
				$this->data['tax'] = $this->currency->format((float)$product_info['special'] ? $product_info['special'] : $product_info['price']);
			}else{
...
// Controllo e aggiungo i prodotti correlati
				$related_is_grouped = $this->model_catalog_product_configurable->getProductRelatedIsGrouped($result['product_id']);
...
// S 5/7 //
		...
		//S switch
			...
			if ($pgd = $this->model_catalog_product_configurable->getProductGroupedDiscount($product_id)) {
				if ($pgd['type'] == 'F') {
			...
			if (isset($this->request->get['error'])) {
				switch ($this->request->get['error']) {
					case 1: $this->data['error_configuration'] = $this->language->get('error_configuration_empty'); break;
					case 2: $this->data['error_configuration'] = $this->language->get('error_configuration_quantity'); break;
					case 3: $this->data['error_configuration'] = $this->language->get('error_configuration_required'); break;
					case 4: $this->data['error_configuration'] = $this->language->get('error_configuration_weight_under'); break;
					case 5: $this->data['error_configuration'] = $this->language->get('error_configuration_weight_over'); break;
				}
			} else {
				$this->data['error_configuration'] = false;
			}
			
			if (isset($this->request->get['cqty'])) {
				$this->data['minimum'] = $this->request->get['cqty'];
			} else {
				$this->data['minimum'] = 1;
			}
			...
			if (isset($this->request->get['cset'])) {
				...
				$cc_find_a = strpos($cc_decode, 'a:'); $cc_find_b = strpos($cc_decode, ':{'); $cc_find_c = strpos($cc_decode, ';}');
			...
			$this->data['action_add_config'] = $this->url->link('checkout/cart/addConfig');
			...
			$this->data['text_qty'] = $this->language->get('text_qty');
			$this->data['toggle_info'] = $this->config->get('use_child_toggle_info_config');
		...
		////////
			...
			$this->data['product_grouped'] = array();
			$this->data['required_fields'] = false;
			$product_configurable = $this->model_catalog_product_configurable->getProductConfigurable($product_id);

			foreach ($product_configurable as $key => $option_type) {
				if ($option_type['child_to_hide']) {
					$childs_to_hide = explode(';', $option_type['child_to_hide']);
					foreach ($childs_to_hide as $element) {
						if (!empty($element)) {
				...
				$childs_id = explode(',', $option_type['child_id']);
				$gp_products = array();
				$compare_id = '';
				$compare_id_count = 0;

				foreach ($childs_id as $child_id) if(!empty($child_id)) {
					$product_info = $this->model_catalog_product->getProduct($child_id);
					...
					if ($product_info) {
						if ($product_info['image']) {
							...
							if ($use_subimage_additional) {
								$results = $this->model_catalog_product->getProductImages($child_id);
								foreach ($results as $result) {
									$this->data['images'][] = array(
										'popup' => $this->model_tool_image->resize($result['image'], $image_popup_w, $image_popup_h),
										'thumb' => $this->model_tool_image->resize($result['image'], $image_additional_w, $image_additional_h),
			...
			if ($this_data_options) {
				$gp_column_option = '1';
						...
						// Disable add to cart
						$child_stock_status_id = $this->model_catalog_product_configurable->getChildStockStatusId($product_id, $child_id);
						if ($product_info['quantity'] <= 0 && $product_info['stock_status_id'] == $child_stock_status_id) {
							$out_of_stock = true;
						} else {
							$out_of_stock = false;
						}
						...
						$this->data['product_grouped'][] = array('product_id' => $product_info['product_id'], 'name' => $name);//individual review

						if($compare_id_count > 0){
							$compare_id .= '-' . $product_info['product_id'];
						}else{
							$compare_id .= $product_info['product_id'];
						}
						$compare_id_count++;
				...
				$option_qty_range = explode(':', $option_type['option_min_qty']);
				if (isset($option_qty_range[1])) {
					$config_option_qty_min = $option_qty_range[0];
					$config_option_qty_max = $option_qty_range[1];
				} else {
					$config_option_qty_min = $option_type['option_min_qty'];
					$config_option_qty_max = '';
				}

				$option_type_switch = substr($option_type['option_type'],0,1);
				if ($option_type_switch != 'n' && $option_type['option_name']) {
					$config_option_name = $option_type['option_name'];
				...
				$this->data['config_options'][] = array(
					...
					'option_required'    => $option_type['option_required'],
					'option_qty_min'     => $config_option_qty_min,
					'option_qty_max'     => $config_option_qty_max,
					'option_hide_qty'    => $option_type['option_hide_qty'],
				...
				if ($option_type['option_required']) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/product_configurable_' . $gp_tpl_q['pg_layout'] . '.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/product/product_configurable_' . $gp_tpl_q['pg_layout'] . '.tpl';
			} else {
				$this->template = 'default/template/product/product_configurable_' . $gp_tpl_q['pg_layout'] . '.tpl';
			}
// E 5/7 //
