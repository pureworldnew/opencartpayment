<?php
class ControllerApiOqOrder extends Controller {
	public function add() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if (!$json && !isset($this->session->data['payment_method'])) {
				$json['error'] = $this->language->get('error_payment_method');
			}
			...
			if (!$json && $this->cart->hasShipping()) {
				...
				// Shipping Method
				if (!$json &&  !isset($this->session->data['shipping_method'])) {
					$json['error'] = $this->language->get('error_shipping_method');
				}
			} else {
	...
	public function addincoming() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if (!$json) {
				...
				$json['order_id'] = $this->model_checkout_order->addIncomingOrder($order_data);
	...
	public function edit() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($order_info) {
				...
				// Cart
				if ((!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
					//$json['error'] = "this is the error";//$this->language->get('error_stock');
					$json['error'][] = $this->cart->hasProducts();//"this is the error";//$this->language->get('error_stock');
//$json['error'][] = $this->session->data['vouchers'];//"this is the error";//$this->language->get('error_stock');
					$json['error'][] = $this->cart->hasStock();//"this is the error";//$this->language->get('error_stock');
					$json['error'][] = $this->config->get('config_stock_checkout');//"this is the error";//$this->language->get('error_stock');
					$json['error'][] = $this->cart->getProducts();//"this is the error";//$this->language->get('error_stock');
				}
				...
				if (!$json) {
					...
					//echo "<pre>"; print_r($order_products); echo "</pre>"; exit;
					...
					$order_data['products'][] = array(
						...
						'subtract'   => $product['subtract'],
						'price'	 	 => isset($order_products[0]['price']) ? $order_products[0]['price'] : $product['price'],
						'total'      => $product['total'],
						'tax'        => $this->tax->getTax($product['price'], $product['tax_class_id']),
						'default_vendor_unit' => !empty($order_products[0]['default_vendor_unit']) ? $order_products[0]['default_vendor_unit'] : $product['default_vendor_unit'],
						'updated_vendor_unit' => !empty($order_products[0]['updated_vendor_unit']) ? $order_products[0]['updated_vendor_unit'] : $product['default_vendor_unit'],
						'remark' => $order_products[0]['remark'],
						'reward'     => $product['reward']
					);
					}
					...
					$order_data['total'] = $this->model_api_order->getOrderGrandTotal($order_id);
