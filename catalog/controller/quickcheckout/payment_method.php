...
class ControllerQuickCheckoutPaymentMethod extends Controller {
  	public function index() {
		...
		if ($this->customer->isLogged() && isset($this->request->get['address_id'])) {
			...
			$this->session->data['order_comment'] = $this->model_account_address->getComments();
		...
		if (!empty($payment_address)) {
			...
			foreach ($results as $result) {
				if ($this->config->get($result['code'] . '_status') && $result['code'] != 'in_store') {
			...
			$json['methoddata']=$method_data;
             $json['cart_total']=$this->cart->getTotal();
			 $json['shipping_address']=isset($this->session->data['shipping_address']) ? $this->session->data['shipping_address'] : "";
		...
		if ($this->customer->isLogged() && isset($this->request->get['address_id'])) {
			...
			$this->session->data['payment_address'] = $this->model_account_address->getAddress($this->request->get['address_id']);
			 $json['cart_total']=$this->cart->getTotal();
					...
					$this->cart->writeLogForOrder('payment_address',serialize($json));
